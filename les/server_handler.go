...
import (
	...
	lps "github.com/tos-network/gtos/les/lespay/server"
	...
	"github.com/tos-network/gtos/p2p/nodestate"
...
func (h *serverHandler) handle(p *clientPeer) error {
	p.Log().Debug("Light Tosnetwk peer connected", "name", p.Name())
	...
	if err := p.Handshake(td, hash, number, h.blockchain.Genesis().Hash(), forkID, h.forkFilter, h.server); err != nil {
		p.Log().Debug("Light Tosnetwk handshake failed", "err", err)
		return err
	}
	...
	if cap, err := h.server.clientPool.connect(p); cap != p.fcParams.MinRecharge || err != nil {
		p.Log().Debug("Light Tosnetwk peer rejected", "err", errFullClientPool)
	...
	for {
		select {
		case err := <-p.errCh:
			p.Log().Debug("Failed to send light tosnetwk response", "err", err)
			return err
		default:
		}
		if err := h.handleMsg(p, &wg); err != nil {
			p.Log().Debug("Light Tosnetwk message handling failed", "err", err)
			return err
		}
	}
}
...
func (h *serverHandler) handleMsg(p *clientPeer, wg *sync.WaitGroup) error {
	...
	p.Log().Trace("Light Tosnetwk message arrived", "code", msg.Code, "bytes", msg.Size)
