...
type TxPool struct {
	...
	// istanbul and eip2718 are always true in GTOS (all blocks are post-Istanbul/Berlin).
...
func (pool *TxPool) setNewHead(head *types.Header) {
	...
	// Fork indicators: always post-Istanbul/Berlin in GTOS (no-op update).
...
func (pool *TxPool) validateTx(ctx context.Context, tx *types.Transaction) error {
	...
	gas, err := core.IntrinsicGas(tx.Data(), tx.AccessList(), tx.To() == nil, true, true)
	if err != nil {
		return err
	}
	if tx.Gas() < gas {
