...
type testOdr struct {
	...
	sdb, ldb      tosdb.Database
	disable       bool
}

func (odr *testOdr) Database() tosdb.Database {
	return odr.ldb
}
...
type odrTestFn func(ctx context.Context, db tosdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error)

func TestOdrGetBlockLes2(t *testing.T) { testChainOdr(t, 1, odrGetBlock) }

func odrGetBlock(ctx context.Context, db tosdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error) {
...
func odrGetReceipts(ctx context.Context, db tosdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error) {
...
func odrAccounts(ctx context.Context, db tosdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error) {
...
func odrContractCall(ctx context.Context, db tosdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error) {
	...
	for i := 0; i < 3; i++ {
		...
		context := core.NewTVMBlockContext(header, chain, nil)
		gp := new(core.GasPool).AddGas(math.MaxUint64)
		result, _ := core.ApplyMessage(context, config, msg, gp, st)
...
func testChainOdr(t *testing.T, protocol int, fn odrTestFn) {
	...
	blockchain, _ := core.NewBlockChain(sdb, nil, params.TestChainConfig, dpos.NewFaker(), nil, nil)
	...
	lightchain, err := NewLightChain(odr, params.TestChainConfig, dpos.NewFaker(), nil)
	if err != nil {
		t.Fatal(err)
	}
