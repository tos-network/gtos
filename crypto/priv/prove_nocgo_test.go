...
func TestProveAndVerifyShieldCTAndBalanceWithContext_NoCgo(t *testing.T) {
...
func TestProveAndVerifyCommitmentEqProofWithAndWithoutContext_NoCgo(t *testing.T) {
	senderPub, senderPriv, err := GenerateKeypair()
	if err != nil {
		t.Fatalf("GenerateKeypair(sender): %v", err)
	}
...
func TestProveAndVerifyRangeProof_NoCgo(t *testing.T) {
	amount := uint64(42)
