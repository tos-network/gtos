#ifndef HEADER_at_src_ballet_ed25519_avx512_at_r43x6_inl_h
#define HEADER_at_src_ballet_ed25519_avx512_at_r43x6_inl_h

#ifndef HEADER_at_src_ballet_ed25519_avx512_at_r43x6_h
#error "Do not include this directly; use at_r43x6.h"
#endif
...
/* Protocols like ED25519 do many GF(p) operations that can be run
   ...
   (e.g. to get at the ILP in, for example, 3 independent at_r43x6_mul,
   ...
   akin to at_r43x6_mul(x,x) vs at_r43x6_sqr(x).
...
/* AT_R43X6_QUAD_DECL(Q) declares the wwl_t's Q03, Q14 and Q25 in the
   local scope to represent at_r43x6_t X, Y, Z and T, but in a more
...
#define AT_R43X6_QUAD_DECL( Q ) wwl_t Q##03, Q##14, Q##25

/* AT_R43X6_QUAD_MOV( D, S ) does D = S.  D and S are AT_R43X6_QUAD
   declarations in the local scope. */

#define AT_R43X6_QUAD_MOV( D, S ) do { D##03 = S##03; D##14 = S##14; D##25 = S##25; } while(0)

/* AT_R43X6_QUAD_PACK(Q,x,y,z,t) does Q = (x,y,z,t) where Q is a
   AT_R43X6_QUAD declared in the local scope, x, y, z and t are
   arbitrary at_r43x6_t. */

#define AT_R43X6_QUAD_PACK( Q, x,y,z,t ) do {                           \
    wwl_t _r0 = (x);                                                    \
    wwl_t _r1 = (y);                                                    \
    wwl_t _r2 = (z);                                                    \
    wwl_t _r3 = (t);                                                    \
...
/* AT_R43X6_QUAD_UNPACK(x,y,z,t,Q) does (x,y,z,t) = Q where x, y, z and
   t are arbitrary at_r43x6_t and Q is a AT_R43X6_QUAD declared in the
   local scope. */

#define AT_R43X6_QUAD_UNPACK( x,y,z,t, Q ) do {               \
    wwl_t _r0 = Q##03;                                        \
    wwl_t _r1 = Q##14;                                        \
    wwl_t _r2 = Q##25;                                        \
    wwl_t _r3 = wwl_zero();                                   \
...
/* AT_R43X6_QUAD_PERMUTE(D,S) does:
     D = [ S(imm0) S(imm1) S(imm2) S(imm3) ]
   where imm* are in [0,3] (0/1/2/3->X/Y/Z/T) */

#define AT_R43X6_QUAD_PERMUTE( D, imm0,imm1,imm2,imm3, S ) do {                                  \
...
/* AT_R43X6_QUAD_LANE_IF does:
     D = [ imm0 ? SX : TX, imm1 ? SY : TY, imm2 ? SZ : TZ, imm3 ? ST : TT ]
   imm* should be in [0,1]. */

#define AT_R43X6_QUAD_LANE_IF( D, imm0,imm1,imm2,imm3, S, T ) do { \
...
/* AT_R43X6_QUAD_LANE_ADD_FAST does:
     D = [ (imm0 ? (PX+QX) : SX) (imm1 ? (PY+QY) : SY) (imm2 ? (PZ+QZ) : SZ) (imm3 ? (PT+QT) : ST) ]
   imm* should be in [0,1]. */

#define AT_R43X6_QUAD_LANE_ADD_FAST( D, S, imm0,imm1,imm2,imm3, P, Q ) do { \
    int _mask = 17*(imm0) + 34*(imm1) + 68*(imm2) + 136*(imm3);             \
...
/* AT_R43X6_QUAD_LANE_SUB_FAST does:
     D = [ (imm0 ? (PX-QX) : SX) (imm1 ? (PY-QY) : SY) (imm2 ? (PZ-QZ) : SZ) (imm3 ? (PT-QT) : ST) ]
   imm* should be in [0,1]. */
#define AT_R43X6_QUAD_LANE_SUB_FAST( D, S, imm0,imm1,imm2,imm3, P, Q ) do { \
    int _mask = 17*(imm0) + 34*(imm1) + 68*(imm2) + 136*(imm3); \
    AT_R43X6_QUAD_DECL( M );                                    \
...
/* AT_R43X6_QUAD_FOLD_UNSIGNED(R,P) does:
     R = [ at_r43x6_fold_unsigned(PX) at_r43x6_fold_unsigned(PY) at_r43x6_fold_unsigned(PZ) at_r43x6_fold_unsigned(PT) ] */

#define AT_R43X6_QUAD_FOLD_UNSIGNED( R, P ) do {                                            \
...
/* AT_R43X6_QUAD_FOLD_SIGNED(R,P) does:
     R = [ at_r43x6_fold_signed(PX) at_r43x6_fold_signed(PY) at_r43x6_fold_signed(PZ) at_r43x6_fold_signed(PT) ] */
#define AT_R43X6_QUAD_FOLD_SIGNED( R, P ) do {                                                                \
...
/* AT_R43X6_QUAD_MUL_FAST(R,P,Q) does (
     [ at_r43x6_mul_fast(PX,QX) at_r43x6_mul_fast(PY,QY) at_r43x6_mul_fast(PZ,QZ) at_r43x6_mul_fast(PT,QT) ]
   Written this way so that pointer escapes don't inhibit optimizations. */

#define AT_R43X6_QUAD_MUL_FAST( R, P, Q ) do {                                                                   \
    AT_R43X6_QUAD_DECL( _R ); at_r43x6_quad_mul_fast( &_R03,&_R14,&_R25, P##03,P##14,P##25, Q##03,Q##14,Q##25 ); \
    AT_R43X6_QUAD_MOV( R, _R );                                                                                  \
  } while(0)

AT_FN_UNUSED static void /* let compiler decide if worth inlining */
at_r43x6_quad_mul_fast( at_r43x6_t * _z03, at_r43x6_t * _z14, at_r43x6_t * _z25,
                        at_r43x6_t    x03, at_r43x6_t    x14, at_r43x6_t    x25,
                        at_r43x6_t    y03, at_r43x6_t    y14, at_r43x6_t    y25 ) {
  ...
  /* Grade school-ish from the original mul:
     ...
     We start with 3 8-lane vectors per input.  These hold 4 at_r43x6_t
     organized as:
     ...
     into two AT_R43X6_QUAD representations at the same time.
...
# endif
  ...
  AT_R43X6_QUAD_MOV( *_z, z );
}

/* AT_R43X6_QUAD_SQR_FAST(R,P) does:
     [ at_r43x6_sqr_fast(PX) at_r43x6_sqr_fast(PY) at_r43x6_sqr_fast(PZ) at_r43x6_sqr_fast(PT) ]
   Written this way so that pointer escapes don't inhibit optimizations. */

#define AT_R43X6_QUAD_SQR_FAST( R, P ) do {                                                   \
    AT_R43X6_QUAD_DECL( _R ); at_r43x6_quad_sqr_fast( &_R03,&_R14,&_R25, P##03,P##14,P##25 ); \
    AT_R43X6_QUAD_MOV( R, _R );                                                               \
  } while(0)

AT_FN_UNUSED static void /* let compiler decide if worth inlining */
at_r43x6_quad_sqr_fast( at_r43x6_t * _z03, at_r43x6_t * _z14, at_r43x6_t * _z25,
                        at_r43x6_t    x03, at_r43x6_t    x14, at_r43x6_t    x25 ) {
...
# endif
  ...
  /* At this point:
     ...
     We complete the calc exactly like AT_R43X6_QUAD_MUL above. */
...
/* Below, AT_R43X6_MUL4_INL( za,xa,ya, zb,xb,yb, zc,xc,yc, zd,xd,yd )
   exactly does:

     za = at_r43x6_mul( xa, ya );
     zb = at_r43x6_mul( xb, yb );
     zc = at_r43x6_mul( xc, yc );
     zd = at_r43x6_mul( xd, yd );

   Likewise, AT_R43X6_SQR4_INL( za,xa, zb,xb, zc,xc, zd,xd ) exactly does:
   ...
   And, AT_R43X6_POW25223_2_INL( za,xa, zb,xb ) exactly does:

     za = at_r43x6_pow25223( xa );
     zb = at_r43x6_pow25223( xb );

   Similarly for AT_R43X6_MUL{1,2,3}_INL, AT_R43X6_SQR{1,2,3}_INL and
   AT_R43X6_POW25223_1_INL( za ).
   ...
   allows higher performance akin to what was done for at_r43x6_sqr.
...
#define AT_R43X6_MUL1_INL( za,xa,ya ) do { \
    (za) = at_r43x6_mul( (xa), (ya) );     \
  } while(0)

#define AT_R43X6_MUL2_INL( za,xa,ya, zb,xb,yb ) do { \
    (za) = at_r43x6_mul( (xa), (ya) );               \
    (zb) = at_r43x6_mul( (xb), (yb) );               \
  } while(0)

#define AT_R43X6_MUL3_INL( za,xa,ya, zb,xb,yb, zc,xc,yc ) do { \
    (za) = at_r43x6_mul( (xa), (ya) );                         \
    (zb) = at_r43x6_mul( (xb), (yb) );                         \
    (zc) = at_r43x6_mul( (xc), (yc) );                         \
  } while(0)

#define AT_R43X6_MUL4_INL( za,xa,ya, zb,xb,yb, zc,xc,yc, zd,xd,yd ) do { \
    (za) = at_r43x6_mul( (xa), (ya) );                                   \
    (zb) = at_r43x6_mul( (xb), (yb) );                                   \
    (zc) = at_r43x6_mul( (xc), (yc) );                                   \
    (zd) = at_r43x6_mul( (xd), (yd) );                                   \
...
#define AT_R43X6_SQR1_INL( za,xa ) do { \
    (za) = at_r43x6_sqr( (xa) );        \
  } while(0)

#define AT_R43X6_SQR2_INL( za,xa, zb,xb ) do { \
    (za) = at_r43x6_sqr( (xa) );               \
    (zb) = at_r43x6_sqr( (xb) );               \
  } while(0)

#define AT_R43X6_SQR3_INL( za,xa, zb,xb, zc,xc ) do { \
    (za) = at_r43x6_sqr( (xa) );                      \
    (zb) = at_r43x6_sqr( (xb) );                      \
    (zc) = at_r43x6_sqr( (xc) );                      \
  } while(0)

#define AT_R43X6_SQR4_INL( za,xa, zb,xb, zc,xc, zd,xd ) do { \
    (za) = at_r43x6_sqr( (xa) );                             \
    (zb) = at_r43x6_sqr( (xb) );                             \
    (zc) = at_r43x6_sqr( (xc) );                             \
    (zd) = at_r43x6_sqr( (xd) );                             \
...
#define AT_R43X6_POW22523_1_INL( za,xa ) do { \
    (za) = at_r43x6_pow22523( (xa) );         \
  } while(0)

#define AT_R43X6_POW22523_2_INL( za,xa, zb,xb ) do { \
    (za) = at_r43x6_pow22523( (xa) );                \
    (zb) = at_r43x6_pow22523( (xb) );                \
  } while(0)
...
#define AT_R43X6_MUL1_INL( z,x,y ) do { \
    (z) = at_r43x6_mul( (x), (y) );     \
  } while(0)
...
#define AT_R43X6_MUL3_INL( za,xa,ya, zb,xb,yb, zc,xc,yc ) do {                                   \
    AT_R43X6_QUAD_DECL( _X ); AT_R43X6_QUAD_PACK         ( _X, (xa),(xb),(xc),at_r43x6_zero() ); \
    AT_R43X6_QUAD_DECL( _Y ); AT_R43X6_QUAD_PACK         ( _Y, (ya),(yb),(yc),at_r43x6_zero() ); \
    AT_R43X6_QUAD_DECL( _Z ); AT_R43X6_QUAD_MUL_FAST     (  _Z, _X, _Y );                        \
    /**/                      AT_R43X6_QUAD_FOLD_UNSIGNED( _Z, _Z );                             \
    at_r43x6_t _zd;           AT_R43X6_QUAD_UNPACK       ( (za),(zb),(zc),_zd, _Z );             \
    (void)_zd;                                                                                   \
  } while(0)
...
#define AT_R43X6_MUL4_INL( za,xa,ya, zb,xb,yb, zc,xc,yc, zd,xd,yd ) do {              \
    AT_R43X6_QUAD_DECL( _X ); AT_R43X6_QUAD_PACK         ( _X, (xa),(xb),(xc),(xd) ); \
    AT_R43X6_QUAD_DECL( _Y ); AT_R43X6_QUAD_PACK         ( _Y, (ya),(yb),(yc),(yd) ); \
    AT_R43X6_QUAD_DECL( _Z ); AT_R43X6_QUAD_MUL_FAST     (  _Z, _X, _Y );             \
    /**/                      AT_R43X6_QUAD_FOLD_UNSIGNED( _Z, _Z );                  \
    /**/                      AT_R43X6_QUAD_UNPACK       ( (za),(zb),(zc),(zd), _Z ); \
  } while(0)

/* Nothing to interleave so let compiler decide */

#define AT_R43X6_SQR1_INL( z,x ) do { (z) = at_r43x6_sqr( (x) ); } while(0)
...
#define AT_R43X6_SQR4_INL( za,xa, zb,xb, zc,xc, zd,xd ) do {                          \
    AT_R43X6_QUAD_DECL( _X ); AT_R43X6_QUAD_PACK         ( _X, (xa),(xb),(xc),(xd) ); \
    AT_R43X6_QUAD_DECL( _Z ); AT_R43X6_QUAD_SQR_FAST     ( _Z, _X );                  \
    /**/                      AT_R43X6_QUAD_FOLD_UNSIGNED( _Z, _Z );                  \
    /**/                      AT_R43X6_QUAD_UNPACK       ( (za),(zb),(zc),(zd), _Z ); \
  } while(0)
...
#define AT_R43X6_POW22523_2_INL( za,xa, zb,xb ) do { \
    at_r43x6_t _za; at_r43x6_t _zb;                  \
    at_r43x6_pow22523_2( &_za,(xa), &_zb,(xb) );     \
    (za) = _za; (zb) = _zb;                          \
  } while(0)
...
#endif /* HEADER_at_src_ballet_ed25519_avx512_at_r43x6_inl_h */
