/* at_merlin.c - Merlin transcript implementation

   Based on STROBE-128 for Fiat-Shamir heuristic in zero-knowledge proofs.
...
static inline void
strobe128_run_f( at_merlin_strobe128_t * ctx ) {
  ctx->state_bytes[ctx->pos] ^= ctx->pos_begin;
  ctx->state_bytes[ctx->pos + 1] ^= 0x04;
  ctx->state_bytes[STROBE_R + 1] ^= 0x80;
  at_keccak256_core( ctx->state );
  ctx->pos = 0;
  ctx->pos_begin = 0;
}

static void
strobe128_absorb( at_merlin_strobe128_t * ctx,
                  uchar const *           data,
...
static void
strobe128_squeeze( at_merlin_strobe128_t * ctx,
                   uchar *                 data,
                   ulong const             data_len) {
  for ( ulong i=0; i<data_len; i++ ) {
...
static void
strobe128_begin_op( at_merlin_strobe128_t * ctx,
                    uchar                   flags ) {
...
static inline void
strobe128_meta_ad( at_merlin_strobe128_t * ctx,
                   uchar const *           data,
                   ulong                   data_len,
                   uchar                   more ) {
  if ( more==0 ) {
...
static inline void
strobe128_ad(at_merlin_strobe128_t * ctx,
             uchar const *           data,
             ulong const             data_len,
             uchar                   more) {
  if ( more==0 ) {
...
static inline void
strobe128_prf( at_merlin_strobe128_t * ctx,
               uchar *                 data,
               ulong const             data_len,
               uchar                   more ) {
  if ( more==0 ) {
...
static inline void
strobe128_init( at_merlin_strobe128_t * ctx,
                uchar const *           label,
  ...
  at_memset( ctx->state_bytes, 0, 200 );
  at_memcpy( ctx->state_bytes, init, 18 );
  at_keccak256_core( ctx->state );
  ctx->pos = 0;
  ctx->pos_begin = 0;
  ctx->cur_flags = 0;
...
at_merlin_transcript_init( at_merlin_transcript_t * mctx,
  ...
  strobe128_init(&mctx->sctx, (uchar *)AT_MERLIN_LITERAL("Merlin v1.0"));
  at_merlin_transcript_append_message(mctx, AT_MERLIN_LITERAL("dom-sep"), (uchar *)label, label_len);
}
...
at_merlin_transcript_append_u64( at_merlin_transcript_t * mctx,
  ...
  at_merlin_transcript_append_message( mctx, label, label_len, (uchar *)&message_u64, 8 );
}
