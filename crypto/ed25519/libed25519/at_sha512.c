...
at_sha512_align( void ) {
  return AT_SHA512_ALIGN;
}

ulong
at_sha512_footprint( void ) {
  return AT_SHA512_FOOTPRINT;
}

void *
at_sha512_new( void * shmem ) {
  at_sha512_t * sha = (at_sha512_t *)shmem;
  ...
  if( AT_UNLIKELY( !at_ulong_is_aligned( (ulong)shmem, at_sha512_align() ) ) ) {
    AT_LOG_WARNING(( "misaligned shmem" ));
    return NULL;
  }

  ulong footprint = at_sha512_footprint();

  at_memset( sha, 0, footprint );

  AT_COMPILER_MFENCE();
  AT_VOLATILE( sha->magic ) = AT_SHA512_MAGIC;
  AT_COMPILER_MFENCE();
...
at_sha512_join( void * shsha ) {
  ...
  if( AT_UNLIKELY( !at_ulong_is_aligned( (ulong)shsha, at_sha512_align() ) ) ) {
    AT_LOG_WARNING(( "misaligned shsha" ));
    return NULL;
  }

  at_sha512_t * sha = (at_sha512_t *)shsha;

  if( AT_UNLIKELY( sha->magic!=AT_SHA512_MAGIC ) ) {
...
#if AT_HAS_AVX
#define AT_SHA512_CORE_IMPL 1
#else
#define AT_SHA512_CORE_IMPL 0
#endif
#endif

#if AT_SHA512_CORE_IMPL==0
...
/* The implementation below was derived from OpenSSL's sha512
   ...
   been streamlined specifically for use inside Avatar base machine
   model (no machine specific capabilities required).

   In particular, at_sha512_core_ref is based on OpenSSL's
...
static void
at_sha512_core_ref( ulong *       state,        /* 64-byte aligned, 8 entries */
...
# define ROTR       at_ulong_rotate_right
...
#define at_sha512_core at_sha512_core_ref

#elif AT_SHA512_CORE_IMPL==1

__attribute__((sysv_abi))
void
at_sha512_core_avx2( ulong *       state,       /* 64-byte aligned, 8 entries */
                     uchar const * block,       /* ideally 128-byte aligned (but not required), 128*block_cnt in size */
                     ulong         block_cnt ); /* positive */

#define at_sha512_core at_sha512_core_avx2

#else
#error "Unsupported AT_SHA512_CORE_IMPL"
#endif
...
at_sha512_append( at_sha512_t * sha,
  ...
  if( AT_UNLIKELY( buf_used ) ) { /* optimized for well aligned use of append */
    ...
    ulong buf_rem = AT_SHA512_PRIVATE_BUF_MAX - buf_used; /* In (0,AT_SHA512_PRIVATE_BUF_MAX) */
    ...
    at_sha512_core( state, buf, 1UL );
    sha->buf_used = 0UL;
  }

  /* Append the bulk of the data */

  ulong block_cnt = sz >> AT_SHA512_PRIVATE_LG_BUF_MAX;
  if( AT_LIKELY( block_cnt ) ) at_sha512_core( state, data, block_cnt ); /* optimized for large append */

  /* Buffer any leftover bytes */

  buf_used = sz & (AT_SHA512_PRIVATE_BUF_MAX-1UL); /* In [0,AT_SHA512_PRIVATE_BUF_MAX) */
  if( AT_UNLIKELY( buf_used ) ) { /* optimized for well aligned use of append */
    at_memcpy( buf, data + (block_cnt << AT_SHA512_PRIVATE_LG_BUF_MAX), buf_used );
    sha->buf_used = buf_used; /* In (0,AT_SHA512_PRIVATE_BUF_MAX) */
  }
...
at_sha512_fini( at_sha512_t * sha,
  ...
  ulong * state      = sha->state;
  uchar * buf        = sha->buf;
  ulong   buf_used   = sha->buf_used; /* In [0,AT_SHA512_PRIVATE_BUF_MAX) */
  ulong   bit_cnt_lo = sha->bit_cnt_lo;
  ulong   bit_cnt_hi = sha->bit_cnt_hi;
  ...
  if( AT_UNLIKELY( buf_used > AT_SHA512_PRIVATE_BUF_MAX-16UL ) ) { /* optimize for well aligned use of append */
    at_memset( buf + buf_used, 0, AT_SHA512_PRIVATE_BUF_MAX-buf_used );
    at_sha512_core( state, buf, 1UL );
    buf_used = 0UL;
  }
  ...
  at_memset( buf + buf_used, 0, AT_SHA512_PRIVATE_BUF_MAX-16UL-buf_used );
  *((ulong *)(buf+AT_SHA512_PRIVATE_BUF_MAX-16UL)) = at_ulong_bswap( bit_cnt_hi );
  *((ulong *)(buf+AT_SHA512_PRIVATE_BUF_MAX- 8UL)) = at_ulong_bswap( bit_cnt_lo );
  at_sha512_core( state, buf, 1UL );

  /* Unpack the result into md (annoying bswaps here) */

  ulong * hash = (ulong *)_hash;
  hash[0] = at_ulong_bswap( state[0] );
  hash[1] = at_ulong_bswap( state[1] );
  hash[2] = at_ulong_bswap( state[2] );
  hash[3] = at_ulong_bswap( state[3] );
  hash[4] = at_ulong_bswap( state[4] );
  hash[5] = at_ulong_bswap( state[5] );
  hash[6] = at_ulong_bswap( state[6] );
  hash[7] = at_ulong_bswap( state[7] );
  return _hash;
}

void *
at_sha384_fini( at_sha512_t * sha,
                void *        _hash ) {
  uchar hash[ AT_SHA512_HASH_SZ ] __attribute__((aligned(64)));
  at_sha512_fini( sha, hash );
  at_memcpy( _hash, hash, AT_SHA384_HASH_SZ );
  return _hash;
}

void *
at_sha512_hash( void const * _data,
                ulong        sz,
                void *       _hash ) {
  uchar const * data = (uchar const *)_data;
  ...
  uchar buf[ AT_SHA512_PRIVATE_BUF_MAX ] __attribute__((aligned(128)));
  ...
  ulong block_cnt = sz >> AT_SHA512_PRIVATE_LG_BUF_MAX;
  if( AT_LIKELY( block_cnt ) ) at_sha512_core( state, data, block_cnt );

  ulong buf_used = sz & (AT_SHA512_PRIVATE_BUF_MAX-1UL);
  if( AT_UNLIKELY( buf_used ) ) at_memcpy( buf, data + (block_cnt << AT_SHA512_PRIVATE_LG_BUF_MAX), buf_used );
  buf[ buf_used ] = (uchar)0x80;
  buf_used++;

  if( AT_UNLIKELY( buf_used > (AT_SHA512_PRIVATE_BUF_MAX-16UL) ) ) {
    at_memset( buf + buf_used, 0, AT_SHA512_PRIVATE_BUF_MAX-buf_used );
    at_sha512_core( state, buf, 1UL );
    buf_used = 0UL;
  }
...
void *
at_sha384_hash( void const * _data,
                ulong        sz,
                void *       _hash ) {
  uchar const * data = (uchar const *)_data;
