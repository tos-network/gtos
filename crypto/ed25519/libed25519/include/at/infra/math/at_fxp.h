#ifndef HEADER_at_src_util_math_at_fxp_h
#define HEADER_at_src_util_math_at_fxp_h
...
#if AT_HAS_INT128 /* See the uwide-based implementations for details how these work */

AT_FN_CONST static inline uint128 at_fxp_private_expand( ulong x ) { return ((uint128)x)<<30; }
static inline ulong at_fxp_private_contract( uint128 x, ulong * _c ) { x >>= 30; *_c = (ulong)(x>>64); return (ulong)x; }

/* Return the low 64-bits of a uint128 and store the high 64-bits at *_h */

static inline ulong at_fxp_private_split( uint128 x, ulong * _h ) { *_h = (ulong)(x>>64); return (ulong)x; }
...
static inline void at_fxp_private_expand( ulong * _yh, ulong * _yl, ulong x ) { *_yh = x>>34; *_yl = x<<30; }

/* Helper used by the below that computes
     2^64 c + y = floor( (2^64 xh + xl) / 2^30 )
   (i.e. shift a 128b uint right by 30).  Exact.  c<2^34. */

static inline ulong at_fxp_private_contract( ulong xh, ulong xl, ulong * _c ) { *_c = xh>>30; return (xh<<34) | (xl>>30); }
...
static inline ulong at_fxp_add( ulong x, ulong y, ulong * _c ) { *_c = (ulong)(x>(~y)); return x + y; }
AT_FN_CONST static inline ulong at_fxp_add_fast( ulong x, ulong y ) { return x + y; }
...
static inline ulong at_fxp_sub( ulong x, ulong y, ulong * _b ) { *_b = (ulong)(x<y); return x - y; }
AT_FN_CONST static inline ulong at_fxp_sub_fast( ulong x, ulong y ) { return x - y; }
...
static inline ulong
at_fxp_mul_rtz( ulong   x,
                ulong   y,
                ulong * _c ) {
  return at_fxp_private_contract( ((uint128)x)*((uint128)y), _c );
}

static inline ulong
at_fxp_mul_raz( ulong   x,
                ulong   y,
                ulong * _c ) {
  return at_fxp_private_contract( ((uint128)x)*((uint128)y)+(uint128)((1UL<<30)-1UL), _c );
}

static inline ulong
at_fxp_mul_rnz( ulong   x,
                ulong   y,
                ulong * _c ) {
  return at_fxp_private_contract( ((uint128)x)*((uint128)y)+(uint128)((1UL<<29)-1UL), _c );
}

static inline ulong
at_fxp_mul_rna( ulong   x,
                ulong   y,
                ulong * _c ) {
  return at_fxp_private_contract( ((uint128)x)*((uint128)y)+(uint128)(1UL<<29), _c );
}

static inline ulong
at_fxp_mul_rne( ulong   x,
                ulong   y,
                ulong * _c ) {
  uint128 z = ((uint128)x)*((uint128)y);
  return at_fxp_private_contract( z + (uint128)(((1UL<<29)-1UL) + (((ulong)(z>>30)) & 1UL)), _c );
}

static inline ulong
at_fxp_mul_rno( ulong   x,
                ulong   y,
                ulong * _c ) {
  uint128 z = ((uint128)x)*((uint128)y);
  return at_fxp_private_contract( z + (uint128)((1UL<<29) - (((ulong)(z>>30)) & 1UL)), _c );
}
...
/* rtz -> Round toward zero (aka truncate rounding)
   ...
   multiplication (at_uwide_mul) and a 128b shift right by 30
   (at_fxp_private_contract).
...
at_fxp_mul_rtz( ulong   x,
                ulong   y,
                ulong * _c ) {
  ulong zh,zl; at_uwide_mul( &zh,&zl, x, y ); /* <= 2^128 - 2^65 + 1 so no overflow */
  return at_fxp_private_contract( zh,zl, _c );
}
...
/* raz -> Round away from zero
   ...
   With RAZ rounding:
        z      = ceil( x y / 2^30 )
               ...
               = (x*y + 2^30 -1) >> 30
   As x*y might overflow 64 bits, we need to do a 64b*64b->128b
   multiplication (at_uwide_mul), a 64b increment of a 128b
   (at_uwide_inc) and a 128b shift right by 30
   (at_fxp_private_contract).

   Slightly more expensive (one at_uwide_inc) than RTZ rounding.
...
at_fxp_mul_raz( ulong   x,
                ...
                ulong * _c ) {
  ulong zh,zl; at_uwide_mul( &zh,&zl, x, y );    /* <= 2^128 - 2^65 + 1 so no overflow */
  at_uwide_inc( &zh,&zl, zh,zl, (1UL<<30)-1UL ); /* <= 2^128 - 2^65 + 2^30 so no overflow */
  return at_fxp_private_contract( zh,zl, _c );
}
...
at_fxp_mul_rnz( ulong   x,
                ...
                ulong * _c ) {
  ulong zh,zl; at_uwide_mul( &zh,&zl, x, y ); /* <= 2^128 - 2^65 + 1 so no overflow */
  at_uwide_inc( &zh,&zl, zh,zl, (1UL<<29)-1UL ); /* <= 2^128 - 2^65 + 2^29 so no overflow */
  return at_fxp_private_contract( zh,zl, _c );
}
...
at_fxp_mul_rna( ulong   x,
                ...
                ulong * _c ) {
  ulong zh,zl; at_uwide_mul( &zh,&zl, x, y ); /* <= 2^128 - 2^65 + 1 so no overflow */
  at_uwide_inc( &zh,&zl, zh,zl, 1UL<<29 );    /* <= 2^128 - 2^65 + 2^29 so no overflow */
  return at_fxp_private_contract( zh,zl, _c );
}
...
/* rne -> Round toward nearest with ties toward even (aka banker's rounding / IEEE style rounding)
   ...
   bits, we need to do a 64b*64b->128b multiplication (at_uwide_mul), a
   64b increment of a 128b (at_uwide_inc) and a 128b shift right by 30
   (at_fxp_private_contract).

   The most accurate style of rounding usually and somewhat more
   expensive (some sequentially dependent bit ops and one at_uwide_inc)
...
at_fxp_mul_rne( ulong   x,
                ulong   y,
                ulong * _c ) {
  ulong zh,zl; at_uwide_mul( &zh,&zl, x, y );   /* <= 2^128 - 2^65 + 1 */
  ulong t = ((1UL<<29)-1UL) + ((zl>>30) & 1UL); /* t = 2^29-1 / 2^29 when bit 30 of x*y is 0 / 1 */
  at_uwide_inc( &zh,&zl, zh,zl, t );            /* <= 2^128 - 2^65 + 2^29 */
  return at_fxp_private_contract( zh,zl, _c );
}
...
/* rno -> Round toward nearest with ties toward odd
   ...
   (at_uwide_mul), a 64b increment of a 128b (at_uwide_inc) and a 128b
   shift right by 30 (at_fxp_private_contract).

   Somewhat more expensive (some sequentially dependent bit ops and one
   at_uwide_inc) than RTZ rounding.  Rounding error in [-1/2,1/2] ulp
   (unbiased).  (ulp==2^-30). */
...
AT_FN_CONST static inline ulong at_fxp_mul_rtz_fast( ulong x, ulong y ) { return (x*y)                 >> 30; }
AT_FN_CONST static inline ulong at_fxp_mul_raz_fast( ulong x, ulong y ) { return (x*y+((1UL<<30)-1UL)) >> 30; }
AT_FN_CONST static inline ulong at_fxp_mul_rnz_fast( ulong x, ulong y ) { return (x*y+((1UL<<29)-1UL)) >> 30; }
AT_FN_CONST static inline ulong at_fxp_mul_rna_fast( ulong x, ulong y ) { return (x*y+ (1UL<<29))      >> 30; }

AT_FN_CONST static inline ulong
at_fxp_mul_rne_fast( ulong x,
                     ulong y ) {
  ulong z = x*y;
...
static inline ulong at_fxp_mul_rdn( ulong x, ulong y, ulong * _c ) { return at_fxp_mul_rtz( x, y, _c ); }
static inline ulong at_fxp_mul_rup( ulong x, ulong y, ulong * _c ) { return at_fxp_mul_raz( x, y, _c ); }
static inline ulong at_fxp_mul_rnd( ulong x, ulong y, ulong * _c ) { return at_fxp_mul_rnz( x, y, _c ); }
static inline ulong at_fxp_mul_rnu( ulong x, ulong y, ulong * _c ) { return at_fxp_mul_rna( x, y, _c ); }

AT_FN_CONST static inline ulong at_fxp_mul_rdn_fast( ulong x, ulong y ) { return at_fxp_mul_rtz_fast( x, y ); }
AT_FN_CONST static inline ulong at_fxp_mul_rup_fast( ulong x, ulong y ) { return at_fxp_mul_raz_fast( x, y ); }
AT_FN_CONST static inline ulong at_fxp_mul_rnd_fast( ulong x, ulong y ) { return at_fxp_mul_rnz_fast( x, y ); }
AT_FN_CONST static inline ulong at_fxp_mul_rnu_fast( ulong x, ulong y ) { return at_fxp_mul_rna_fast( x, y ); }
...
static inline ulong
at_fxp_div_rtz( ulong   x,
                ulong   y,
                ulong * _c ) {
  if( !y ) { *_c = ULONG_MAX; return 0UL; }
  return at_fxp_private_split( at_fxp_private_expand( x ) / (uint128)y, _c );
}

static inline ulong
at_fxp_div_raz( ulong   x,
                ulong   y,
                ulong * _c ) {
  if( !y ) { *_c = ULONG_MAX; return 0UL; }
  return at_fxp_private_split( (at_fxp_private_expand( x )+(uint128)(y-1UL)) / (uint128)y, _c );
}

static inline ulong
at_fxp_div_rnz( ulong   x,
                ulong   y,
                ulong * _c ) {
  if( !y ) { *_c = ULONG_MAX; return 0UL; }
  return at_fxp_private_split( (at_fxp_private_expand( x )+(uint128)((y-1UL)>>1)) / (uint128)y, _c );
}

static inline ulong
at_fxp_div_rna( ulong   x,
                ulong   y,
                ulong * _c ) {
  if( !y ) { *_c = ULONG_MAX; return 0UL; }
  return at_fxp_private_split( (at_fxp_private_expand( x )+(uint128)(y>>1)) / (uint128)y, _c );
}

static inline ulong
at_fxp_div_rne( ulong   x,
                ulong   y,
                ulong * _c ) {
  if( !y ) { *_c = ULONG_MAX; return 0UL; }
  uint128 n    = at_fxp_private_expand( x );
  uint128 q    = n / (uint128)y;
  ulong   r    = (ulong)(n - q*y);
  ulong   flhy = y>>1;
  return at_fxp_private_split( q + (uint128)(ulong)( (r>flhy) | ((r==flhy) & !!((~y) & ((ulong)q) & 1UL)) ), _c );
}

static inline ulong
at_fxp_div_rno( ulong   x,
                ulong   y,
                ulong * _c ) {
  if( !y ) { *_c = ULONG_MAX; return 0UL; }
  uint128 n    = at_fxp_private_expand( x );
...
/* rtz -> Round toward zero (aka truncate rounding)
   ...
   (at_fxp_private_expand) and then use a 128b/64b -> 128b divider.
   (at_uwide_div).
...
at_fxp_div_rtz( ulong   x,
  ...
  ulong zh,zl; at_fxp_private_expand( &zh,&zl, x ); /* 2^30 x  <= 2^94-2^30 so no overflow */
  at_uwide_div( &zh,&zl, zh,zl, y );                /* <zh,zl> <= 2^94-2^30 so no overflow */
  *_c = zh; return zl;
}
...
/* raz -> Round away from zero
   ...
   With RAZ rounding:
        z      = ceil( 2^30 x / y )
               = floor( (2^30 x + y - 1) / y )
   As 2^30 x might overflow 64 bits, we need to expand x
   (at_fxp_private_expand), increment it by the 64b y-1 (at_uwide_inc)
   and then use a 128b/64b->128b divider (at_uwide_div).
...
at_fxp_div_raz( ulong   x,
  ...
  if( !y ) { *_c = ULONG_MAX; return 0UL; }         /* Handle divide by zero */
  ulong zh,zl; at_fxp_private_expand( &zh,&zl, x ); /* 2^30 x  <= 2^94-2^30 so no overflow */
  at_uwide_inc( &zh,&zl, zh,zl, y-1UL );            /* <zh,zl> <= 2^94+2^64-2^30-2 so no overflow */
  at_uwide_div( &zh,&zl, zh,zl, y );                /* <zh,zl> = ceil( 2^30 x / y ) <= 2^94-2^30 so no overflow */
  *_c = zh; return zl;
}
...
/* rnz -> Round nearest with ties toward zero
   ...
   (at_fxp_private_expand), increment it by the 64b (y-1)>>1
   (at_uwide_inc) and then use a 128b/64b->128b divider (at_uwide_div).
...
at_fxp_div_rnz( ulong   x,
  ...
  if( !y ) { *_c = ULONG_MAX; return 0UL; }         /* Handle divide by zero */
  ulong zh,zl; at_fxp_private_expand( &zh,&zl, x ); /* 2^30 x <= 2^94-2^30 so no overflow */
  at_uwide_inc( &zh,&zl, zh,zl, (y-1UL)>>1 );       /* <zh,zl> <= 2^94-2^30 + 2^63-1 so no overflow */
  at_uwide_div( &zh,&zl, zh,zl, y );                /* <zh,zl> <= ceil(2^30 x/y) <= 2^94-2^30 so no overflow */
  *_c = zh; return zl;
}
...
at_fxp_div_rna( ulong   x,
  ...
  if( !y ) { *_c = ULONG_MAX; return 0UL; }         /* Handle divide by zero */
  ulong zh,zl; at_fxp_private_expand( &zh,&zl, x ); /* 2^30 x <= 2^94-2^30 so no overflow */
  at_uwide_inc( &zh,&zl, zh,zl, y>>1 );             /* <zh,zl> <= 2^94-2^30 + 2^63-1 so no overflow */
  at_uwide_div( &zh,&zl, zh,zl, y );                /* <zh,zl> <= ceil(2^30 x/y) <= 2^94-2^30 so no overflow */
  *_c = zh; return zl;
}
...
/* rne -> Round nearest with ties toward even (aka banker's rounding / IEEE style rounding)
   ...
   As 2^30 x might overflow 64 bits, we need to expand x
   (at_fxp_private_expand).  Since we need both the 128b quotient and
   the 64b remainder, we need a 128b/64b->128b,64b divider
   (at_uwide_divrem ... if there was a way to quickly determine if
   floor( 2^30 x / y ) is even or odd, we wouldn't need the remainder
   and could select the appropriate RNZ/RNA based at_uwide_inc
   increment) and then a 128b conditional increment (at_uwide_inc).

   The most accurate style of rounding usually and somewhat more
   expensive (needs remainder, some sequentially dependent bit ops and
   one at_uwide_inc) than RTZ rounding.  Rounding error in [-1/2,1/2]
...
at_fxp_div_rne( ulong   x,
  ...
  if( !y ) { *_c = ULONG_MAX; return 0UL; }          /* Handle divide by zero */
  ulong zh,zl; at_fxp_private_expand( &zh,&zl, x );  /* 2^30 x <= 2^94-2^30 so no overflow */
  ulong r    = at_uwide_divrem( &zh,&zl, zh,zl, y ); /* <zh,zl>*y + r = 2^30 x where r is in [0,y-1] so no overflow */
  ulong flhy = y>>1;                                 /* floor(y/2) so no overflow */
  at_uwide_inc( &zh,&zl, zh,zl, (ulong)( (r>flhy) | ((r==flhy) & !!((~y) & zl & 1UL)) ) );
...
/* rno -> Round nearest with ties toward odd
   ...
   bit ops and one at_uwide_inc) than RTZ rounding.  Rounding error in
...
AT_FN_CONST static inline ulong at_fxp_div_rtz_fast( ulong x, ulong y ) { return ( x<<30              ) / y; }
AT_FN_CONST static inline ulong at_fxp_div_raz_fast( ulong x, ulong y ) { return ((x<<30)+ (y-1UL)    ) / y; }
AT_FN_CONST static inline ulong at_fxp_div_rnz_fast( ulong x, ulong y ) { return ((x<<30)+((y-1UL)>>1)) / y; }
AT_FN_CONST static inline ulong at_fxp_div_rna_fast( ulong x, ulong y ) { return ((x<<30)+ (y     >>1)) / y; }

AT_FN_CONST static inline ulong
at_fxp_div_rne_fast( ulong x,
                     ulong y ) {
  ulong n    = x << 30;
  ulong q    = n / y;
  ulong r    = n - q*y;
...
static inline ulong at_fxp_div_rdn( ulong x, ulong y, ulong * _c ) { return at_fxp_div_rtz( x, y, _c ); }
static inline ulong at_fxp_div_rup( ulong x, ulong y, ulong * _c ) { return at_fxp_div_raz( x, y, _c ); }
static inline ulong at_fxp_div_rnd( ulong x, ulong y, ulong * _c ) { return at_fxp_div_rnz( x, y, _c ); }
static inline ulong at_fxp_div_rnu( ulong x, ulong y, ulong * _c ) { return at_fxp_div_rna( x, y, _c ); }

AT_FN_CONST static inline ulong at_fxp_div_rdn_fast( ulong x, ulong y ) { return at_fxp_div_rtz_fast( x, y ); }
AT_FN_CONST static inline ulong at_fxp_div_rup_fast( ulong x, ulong y ) { return at_fxp_div_raz_fast( x, y ); }
AT_FN_CONST static inline ulong at_fxp_div_rnd_fast( ulong x, ulong y ) { return at_fxp_div_rnz_fast( x, y ); }
AT_FN_CONST static inline ulong at_fxp_div_rnu_fast( ulong x, ulong y ) { return at_fxp_div_rna_fast( x, y ); }
...
AT_FN_CONST static inline ulong
at_fxp_sqrt_rtz( ulong x ) {
  if( !x ) return 0UL;
  int s = (63-at_ulong_find_msb( x )) >> 1;
  if( s>15 ) s = 15;
  ulong y = at_ulong_sqrt( x << (s<<1) ) << (15-s);
  if( s==15 ) return y;

  uint128 _x = at_fxp_private_expand( x );
...
AT_FN_CONST static inline ulong
at_fxp_sqrt_raz( ulong x ) {
  if( !x ) return 0UL;
  int s = (63-at_ulong_find_msb( x )) >> 1;
  if( s>15 ) s = 15;
  ulong xl = x << (s<<1);
  ulong y = at_ulong_sqrt( xl ) << (15-s);
  if( s==15 ) return y + (ulong)!!(xl-y*y);

  uint128 _x = at_fxp_private_expand( x ) - (uint128)2;
  uint128 _y =  (uint128)y;
  for(;;) {
    uint128 _z = (_y*_y + _y + _x) / ((_y<<1)-(uint128)1);
    if( _z==_y ) break;
...
at_fxp_sqrt_rnz( ulong x ) {
  ...
  uint128 _x = at_fxp_private_expand( x ) -(uint128)1;
  uint128 _y =  (uint128)y;
  for(;;) {
...
at_fxp_sqrt_rtz( ulong x ) {
  ...
  int s = (63-at_ulong_find_msb( x )) >> 1;         /* lg x in [34,63], 63-lg x in [0,29], s in [0,14] when x>=2^34 */
  if( s>15 ) s = 15;                                /* s==15 when x<2^34 */
  ulong y = at_ulong_sqrt( x << (s<<1) ) << (15-s); /* All shifts well defined */
  if( s==15 ) return y;                             /* No iteration if x<2^34 */

  /* Expand x to 2^30 x for the fixed point iteration */
  ulong xh,xl; at_fxp_private_expand( &xh,&xl, x );
  for(;;) {
    ...
    ulong yh,yl;
    at_uwide_mul( &yh,&yl, y,y+1UL );
    at_uwide_add( &yh,&yl, yh,yl, xh,xl, 0UL );
    at_uwide_div( &yh,&yl, yh,yl, (y<<1)+1UL );
    if( yl==y ) break;
    y = yl;
  }
...
/* raz -> Round away zero
   ...
   at_uwide_divrem in the iteration itself to produce the needed
   residual.
...
at_fxp_sqrt_raz( ulong x ) {
  ...
  at_fxp_private_expand( &xh,&xl, x );
  at_uwide_dec( &xh,&xl, xh, xl, 2UL );
  for(;;) {
    ulong yh,yl;
    at_uwide_mul( &yh,&yl, y, y+1UL );
    at_uwide_add( &yh,&yl, yh,yl, xh,xl, 0UL );
...
/* rnz/rna/rne/rno -> Round nearest with ties toward zero/away zero/toward even/toward odd
   ...
   we can use explicitly compute the remainder or use at_uwide_divrem in
...
at_fxp_sqrt_rnz( ulong x ) {
  ...
  at_fxp_private_expand( &xh,&xl, x );          /* 2^30 x */
  at_uwide_dec( &xh,&xl, xh,xl, 1UL );          /* 2^30 x - 1 */
  for(;;) {
    ulong yh,yl;
    at_uwide_mul( &yh,&yl, y,y+1UL );           /* y^2 + y */
    at_uwide_add( &yh,&yl, yh,yl, xh,xl, 0UL ); /* y^2 + y - 1 + 2^30 x */
    at_uwide_div( &yh,&yl, yh,yl, y<<1 );
    if( yl==y ) break;
    y = yl;
  }
...
AT_FN_CONST static inline ulong at_fxp_sqrt_rna( ulong x ) { return at_fxp_sqrt_rnz( x ); }
AT_FN_CONST static inline ulong at_fxp_sqrt_rne( ulong x ) { return at_fxp_sqrt_rnz( x ); }
AT_FN_CONST static inline ulong at_fxp_sqrt_rno( ulong x ) { return at_fxp_sqrt_rnz( x ); }

AT_FN_CONST static inline ulong at_fxp_sqrt_rtz_fast( ulong x ) { return at_ulong_sqrt( x<<30 ); }

AT_FN_CONST static inline ulong
at_fxp_sqrt_raz_fast( ulong x ) {
  ulong xl = x<<30;
  ulong y  = at_ulong_sqrt( xl );
  ulong r  = xl - y*y;
  return y + (ulong)!!r;
}

AT_FN_CONST static inline ulong
at_fxp_sqrt_rnz_fast( ulong x ) {
  ulong xl = x<<30;
  ulong y  = at_ulong_sqrt( xl );
  ulong r  = xl - y*y;
  return y + (ulong)(r>y);
...
AT_FN_CONST static inline ulong at_fxp_sqrt_rna_fast( ulong x ) { return at_fxp_sqrt_rnz_fast( x ); }
AT_FN_CONST static inline ulong at_fxp_sqrt_rne_fast( ulong x ) { return at_fxp_sqrt_rnz_fast( x ); }
AT_FN_CONST static inline ulong at_fxp_sqrt_rno_fast( ulong x ) { return at_fxp_sqrt_rnz_fast( x ); }
...
AT_FN_CONST static inline ulong at_fxp_sqrt_rdn( ulong x ) { return at_fxp_sqrt_rtz( x ); }
AT_FN_CONST static inline ulong at_fxp_sqrt_rup( ulong x ) { return at_fxp_sqrt_raz( x ); }
AT_FN_CONST static inline ulong at_fxp_sqrt_rnd( ulong x ) { return at_fxp_sqrt_rnz( x ); }
AT_FN_CONST static inline ulong at_fxp_sqrt_rnu( ulong x ) { return at_fxp_sqrt_rnz( x ); }

AT_FN_CONST static inline ulong at_fxp_sqrt_rdn_fast( ulong x ) { return at_fxp_sqrt_rtz_fast( x ); }
AT_FN_CONST static inline ulong at_fxp_sqrt_rup_fast( ulong x ) { return at_fxp_sqrt_raz_fast( x ); }
AT_FN_CONST static inline ulong at_fxp_sqrt_rnd_fast( ulong x ) { return at_fxp_sqrt_rnz_fast( x ); }
AT_FN_CONST static inline ulong at_fxp_sqrt_rnu_fast( ulong x ) { return at_fxp_sqrt_rnz_fast( x ); }
...
at_fxp_log2_approx( ulong x,
                    int * _e ) { /* e, in [-30,33] (==at_ulong_find_msb(x)-30 ... see note above) */
  ...
  int   i = at_ulong_find_msb( x );    /* In [0,63] */
...
#endif /* HEADER_at_src_util_math_at_fxp_h */
