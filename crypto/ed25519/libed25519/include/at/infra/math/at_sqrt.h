#ifndef HEADER_at_src_util_math_at_sqrt_h
#define HEADER_at_src_util_math_at_sqrt_h
...
at_ulong_sqrt( ulong x ) {
  ...
  for(;;) {
    ...
    ulong qy = y>>1;  ulong ry = at_ulong_if( y & 1UL, y+qy+1UL, qy );
...
AT_FN_CONST static inline uchar  at_uchar_sqrt ( uchar  x ) { return (uchar )at_uint_sqrt( (uint)x ); }
AT_FN_CONST static inline ushort at_ushort_sqrt( ushort x ) { return (ushort)at_uint_sqrt( (uint)x ); }

/* These return floor( sqrt( x ) ), undefined behavior for negative x. */

AT_FN_CONST static inline schar at_schar_sqrt( schar x ) { return (schar)at_uchar_sqrt ( (uchar )x ); }
AT_FN_CONST static inline short at_short_sqrt( short x ) { return (short)at_ushort_sqrt( (ushort)x ); }
AT_FN_CONST static inline int   at_int_sqrt  ( int   x ) { return (int  )at_uint_sqrt  ( (uint  )x ); }
AT_FN_CONST static inline long  at_long_sqrt ( long  x ) { return (long )at_ulong_sqrt ( (ulong )x ); }

/* These return the floor( re sqrt(x) ) */

AT_FN_CONST static inline schar at_schar_re_sqrt( schar x ) { return at_schar_if( x>(schar)0,  (schar)at_uchar_sqrt ( (uchar )x ), (schar)0  ); }
AT_FN_CONST static inline short at_short_re_sqrt( short x ) { return at_short_if( x>(short)0,  (short)at_ushort_sqrt( (ushort)x ), (short)0  ); }
AT_FN_CONST static inline int   at_int_re_sqrt  ( int   x ) { return at_int_if  ( x>       0,  (int  )at_uint_sqrt  ( (uint  )x ),        0  ); }
AT_FN_CONST static inline long  at_long_re_sqrt ( long  x ) { return at_long_if ( x>       0L, (long )at_ulong_sqrt ( (ulong )x ),        0L ); }

/* These return the floor( sqrt( |x| ) ) */

AT_FN_CONST static inline schar at_schar_sqrt_abs( schar x ) { return (schar)at_uchar_sqrt ( at_schar_abs( x ) ); }
AT_FN_CONST static inline short at_short_sqrt_abs( short x ) { return (short)at_ushort_sqrt( at_short_abs( x ) ); }
AT_FN_CONST static inline int   at_int_sqrt_abs  ( int   x ) { return (int  )at_uint_sqrt  ( at_int_abs  ( x ) ); }
AT_FN_CONST static inline long  at_long_sqrt_abs ( long  x ) { return (long )at_ulong_sqrt ( at_long_abs ( x ) ); }

AT_PROTOTYPES_END

#endif /* HEADER_at_src_util_math_at_sqrt_h */