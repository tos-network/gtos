...
// NewProtocolManager returns a new Tosnetwk sub protocol manager. The Tosnetwk sub protocol manages peers capable
// with the Tosnetwk network.
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing Tosnetwk peer", "peer", id)

	// Unregister the peer from the downloader and Tosnetwk peer set
...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Tosnetwk protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Tosnetwk peer connected", "name", p.Name())

	// Execute the Tosnetwk handshake
	var (
	...
	if err := p.Handshake(pm.networkID, td, hash, genesis.Hash(), forkID, pm.forkFilter); err != nil {
		p.Log().Debug("Tosnetwk handshake failed", "err", err)
		return err
	}

	// Register the peer locally
	if err := pm.peers.Register(p, pm.removePeer); err != nil {
		p.Log().Error("Tosnetwk peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Tosnetwk message handling failed", "err", err)
			return err
		}
	}
}
...
// NodeInfo represents a short summary of the Tosnetwk sub-protocol metadata
// known about the host peer.
type NodeInfo struct {
	Network    uint64              `json:"network"`    // Tosnetwk network ID (1=Frontier, 2=Morden, Ropsten=3, Rinkeby=4)
