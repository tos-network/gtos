...
func (g *gtosrpc) killAndWait() {
	g.gtos.Kill()
	g.gtos.WaitExit()
}

func (g *gtosrpc) callRPC(result interface{}, method string, args ...interface{}) {
	if err := g.rpc.Call(&result, method, args...); err != nil {
		g.gtos.Fatalf("callRPC %v: %v", method, err)
	}
}

func (g *gtosrpc) addPeer(peer *gtosrpc) {
	g.gtos.Logf("%v.addPeer(%v)", g.name, peer.name)
	enode := peer.getNodeInfo().Enode
	peerCh := make(chan *p2p.PeerEvent)
	sub, err := g.rpc.Subscribe(context.Background(), "admin", peerCh, "peerEvents")
	if err != nil {
		g.gtos.Fatalf("subscribe %v: %v", g.name, err)
	}
	...
	select {
	case ev := <-peerCh:
		g.gtos.Logf("%v received event: type=%v, peer=%v", g.name, ev.Type, ev.Peer)
	case err := <-sub.Err():
		g.gtos.Fatalf("%v sub error: %v", g.name, err)
	case <-timeout:
		g.gtos.Error("timeout adding peer after", dur)
	}
}

// Use this function instead of `g.nodeInfo` directly
func (g *gtosrpc) getNodeInfo() *p2p.NodeInfo {
	if g.nodeInfo != nil {
		return g.nodeInfo
	}
...
func (g *gtosrpc) waitSynced() {
	...
	if err != nil {
		g.gtos.Fatalf("%v syncing: %v", g.name, err)
	}
	defer sub.Unsubscribe()
	timeout := time.After(4 * time.Second)
	select {
	case ev := <-ch:
		g.gtos.Log("'syncing' event", ev)
		syncing, ok := ev.(bool)
		if ok && !syncing {
			break
		}
		g.gtos.Log("Other 'syncing' event", ev)
	case err := <-sub.Err():
		g.gtos.Fatalf("%v notification: %v", g.name, err)
		break
	case <-timeout:
		g.gtos.Fatalf("%v timeout syncing", g.name)
		break
	}
}
...
func startGtosWithIpc(t *testing.T, name string, args ...string) *gtosrpc {
	ipcName := fmt.Sprintf("gtos-%d.ipc", atomic.AddUint32(&nextIPC, 1))
	...
	var err error
	ipcpath := ipcEndpoint(ipcName, g.gtos.Datadir)
...
func initGtos(t *testing.T) string {
	args := []string{"--nousb", "--networkid=42", "init", "./testdata/clique.json"}
	t.Logf("Initializing gtos: %v ", args)
	g := runGtos(t, args...)
	datadir := g.Datadir
	g.WaitExit()
	return datadir
...
func startLightServer(t *testing.T) *gtosrpc {
	datadir := initGtos(t)
	t.Logf("Importing keys to gtos")
	runGtos(t, "--nousb", "--datadir", datadir, "--password", "./testdata/password.txt", "account", "import", "./testdata/key.prv", "--lightkdf").WaitExit()
	account := "0x02f0d131f1f97aef08aec6e3291b957d9efe7105"
	server := startGtosWithIpc(t, "lightserver", "--allow-insecure-unlock", "--datadir", datadir, "--password", "./testdata/password.txt", "--unlock", account, "--mine", "--light.serve=100", "--light.maxpeers=1", "--nodiscover", "--nat=extip:127.0.0.1", "--verbosity=4")
	return server
}

func startClient(t *testing.T, name string) *gtosrpc {
	datadir := initGtos(t)
	return startGtosWithIpc(t, name, "--datadir", datadir, "--nodiscover", "--syncmode=light", "--nat=extip:127.0.0.1", "--verbosity=4")
}
