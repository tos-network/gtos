...
func startGethWithIpc(t *testing.T, name string, args ...string) *gethrpc {
	...
	// We can't know exactly how long gtos will take to start, so we try 10
...
func TestPriorityClient(t *testing.T) {
	// Skip: GTOS uses 32-byte addresses (AddressLength=32); this test uses 20-byte
	// Ethereum addresses which are incompatible with GTOS account model.
	t.Skip("LES integration test incompatible with GTOS 32-byte address model")
