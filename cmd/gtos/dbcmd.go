...
func showLeveldbStats(db tosdb.KeyValueStater) {
...
type snapshotIterator struct {
	init    bool
	account tosdb.Iterator
	storage tosdb.Iterator
}
...
var chainExporters = map[string]func(db tosdb.Database) utils.ChainDataIterator{
	"preimage": func(db tosdb.Database) utils.ChainDataIterator {
		iter := db.NewIterator(rawdb.PreimagePrefix, nil)
		return &preimageIterator{iter: iter}
	},
	"snapshot": func(db tosdb.Database) utils.ChainDataIterator {
...
func dbHasLegacyReceipts(db tosdb.Database, firstIdx uint64) (bool, uint64, error) {
	// Check first block for legacy receipt format
	numAncients, err := db.Ancients()
	if err != nil {
		return false, 0, err
