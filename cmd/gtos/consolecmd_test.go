...
func runMinimalGtos(t *testing.T, args ...string) *testgtos {
	...
	return runGtos(t, append(allArgs, args...)...)
}
...
func TestConsoleWelcome(t *testing.T) {
	...
	gtos := runMinimalGtos(t, "--etherbase", coinbase, "console")
...
func TestAttachWelcome(t *testing.T) {
	...
	if runtime.GOOS == "windows" {
		...
		ipc = filepath.Join(ws, "gtos.ipc")
	}
	// And HTTP + WS attachment
	p := trulyRandInt(1024, 65533) // Yeah, sometimes this will fail, sorry :P
	httpPort = strconv.Itoa(p)
	wsPort = strconv.Itoa(p + 1)
	gtos := runMinimalGtos(t, "--etherbase", "0x8605cdbbdb6d264aa742e77020dcbc58fcdce182",
		"--ipcpath", ipc,
...
func testAttachWelcome(t *testing.T, gtos *testgtos, endpoint, apis string) {
	// Attach to a running gtos note and terminate immediately
	attach := runGtos(t, "attach", endpoint)
