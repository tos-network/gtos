...
const (
	clientIdentifier = "gtos" // Client identifier to advertise over the network
)
...
var (
	...
	app = flags.NewApp(gitCommit, gitDate, "the go-tos command line interface")
...
func init() {
	...
	app.Copyright = "Copyright 2013-2022 The go-tos Authors"
...
// gtos is the main entry point into the system if no special subcommand is ran.
// It creates a default node based on the command line arguments and runs it in
// blocking mode, waiting for it to be shut down.
func gtos(ctx *cli.Context) error {
...
// miner.
func startNode(ctx *cli.Context, stack *node.Node, backend tosapi.Backend, isConsole bool) {
	...
	// Create a client to interact with local gtos node.
	rpcClient, err := stack.Attach()
	if err != nil {
		utils.Fatalf("Failed to attach to self: %v", err)
	}
	tosClient := tosclient.NewClient(rpcClient)
	...
	if ctx.Bool(utils.MiningEnabledFlag.Name) || ctx.Bool(utils.DeveloperFlag.Name) {
		tosBackend, ok := backend.(*tos.TOSAPIBackend)
		if !ok {
