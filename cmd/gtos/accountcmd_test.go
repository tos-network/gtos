...
`
	...
	{
		gtos := runGeth(t, "account", "list", "--datadir", datadir)
		gtos.Expect(want)
		gtos.ExpectExit()
	}
	{
		gtos := runGeth(t, "--datadir", datadir, "account", "list")
		gtos.Expect(want)
		gtos.ExpectExit()
	}
}
...
func TestAccountNew(t *testing.T) {
	gtos := runGeth(t, "account", "new", "--lightkdf")
...
`)
	gtos.ExpectRegexp(`
Signer type of the key:      secp256k1
...
func TestAccountNewSchnorr(t *testing.T) {
	gtos := runGeth(t, "account", "new", "--lightkdf", "--signer", "schnorr")
	defer gtos.ExpectExit()
	gtos.Expect(`
Your new account is locked with a password. Please give a password. Do not forget this password.
!! Unsupported terminal, password will be echoed.
...
`)
	gtos.ExpectRegexp(`
Signer type of the key:      schnorr
Public address of the key:   0x[0-9a-fA-F]{64}
Path of the secret key file: .*UTC--.+--[0-9a-f]{64}
...
func TestAccountHelp(t *testing.T) {
	gtos := runGeth(t, "account", "-h")
	gtos.WaitExit()
	if have, want := gtos.ExitStatus(), 0; have != want {
		t.Errorf("exit error, have %d want %d", have, want)
	}

	gtos = runGeth(t, "account", "import", "-h")
	gtos.WaitExit()
	if have, want := gtos.ExitStatus(), 0; have != want {
		t.Errorf("exit error, have %d want %d", have, want)
	}
...
func importAccountWithExpect(t *testing.T, key string, expected string) {
	...
	gtos := runGeth(t, "--lightkdf", "account", "import", "-password", passwordFile, keyfile)
	defer gtos.ExpectExit()
	gtos.Expect(expected)
}
...
func TestWalletImport(t *testing.T) {
	gtos := runGeth(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer gtos.ExpectExit()
	gtos.Expect(`
!! Unsupported terminal, password will be echoed.
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	gtos.SetTemplateFunc("keypath", func(file string) string {
