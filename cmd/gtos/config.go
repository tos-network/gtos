...
func makeConfigNode(ctx *cli.Context) (*node.Node, gtosConfig) {
	...
	if file := ctx.GlobalString(configFileFlag.Name); file != "" {
		...
		if cfg.Shh != (whisperDeprecatedConfig{}) {
			log.Warn("Deprecated whisper config detected. Whisper has been moved to github.com/tosnetwk/whisper")
		}
	}
...
func checkWhisper(ctx *cli.Context) {
	for _, flag := range whisperFlags {
		if ctx.GlobalIsSet(flag.GetName()) {
			log.Warn("deprecated whisper flag detected. Whisper has been moved to github.com/tosnetwk/whisper")
		}
	}
}

// makeFullNode loads gtos configuration and creates the Tosnetwk backend.
func makeFullNode(ctx *cli.Context) (*node.Node, ethapi.Backend) {
	...
	// Add the Tosnetwk Stats daemon if requested.
	if cfg.Ethstats.URL != "" {
