...
import (
	...
	"github.com/tos-network/gtos/p2p/discv5"
...
var (
	...
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the Tosnetwk protocol")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for Tosnetwk and the faucet")
)
...
type request struct {
	...
	Account common.Address     `json:"account"` // Tosnetwk address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an Tosnetwk light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // Tosnetwk protocol stack
	client *ethclient.Client   // Client connection to the Tosnetwk chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the Tosnetwk light client protocol
	...
	if err != nil {
		return nil, fmt.Errorf("Failed to register the Tosnetwk service: %w", err)
	}
...
// close terminates the Tosnetwk connection and tears down the faucet.
...
func (f *faucet) apiHandler(w http.ResponseWriter, r *http.Request) {
	...
	for {
		...
		// Retrieve the Tosnetwk address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
		...
		default:
			//lint:ignore ST1005 This error is to be displayed in the browser
			err = errors.New("Something funky happened, please open an issue at https://github.com/tos-network/gtos/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
...
// the username, avatar URL and Tosnetwk address to fund on success.
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// and scrape it for the Tosnetwk address and profile URL. We need to load
	...
	if address == (common.Address{}) {
		//lint:ignore ST1005 This error is to be displayed in the browser
		return "", "", common.Address{}, errors.New("No Tosnetwk address found to fund")
	}
	var avatar string
...
// returning the username, avatar URL and Tosnetwk address to fund on success.
func authFacebook(url string) (string, string, common.Address, error) {
	...
	// scrape it for the Tosnetwk address and profile URL.
	res, err := http.Get(url)
	if err != nil {
		return "", "", common.Address{}, err
	}
...
// authNoAuth tries to interpret a faucet request as a plain Tosnetwk address,
