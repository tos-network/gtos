...
Then, run the following command, replacing `<enode>` with the enode of the gtos node:
 ```
...
To run the eth66 protocol test suite, initialize a gtos node as described above and run the following command,
replacing `<enode>` with the enode of the gtos node:
...
[dns-tutorial]: https://gtos.ethereum.org/docs/developers/dns-discovery-setup
