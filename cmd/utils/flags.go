...
func SetShhConfig(ctx *cli.Context, stack *node.Node) {
	if ctx.GlobalIsSet(WhisperEnabledFlag.Name) ||
		...
		log.Warn("Whisper support has been deprecated and the code has been moved to github.com/tosnetwk/whisper")
	}
}
...
// RegisterEthService adds an Tosnetwk client to the stack.
func RegisterEthService(stack *node.Node, cfg *eth.Config) ethapi.Backend {
	if cfg.SyncMode == downloader.LightSync {
		backend, err := les.New(stack, cfg)
		if err != nil {
			Fatalf("Failed to register the Tosnetwk service: %v", err)
		}
...
// RegisterEthStatsService configures the Tosnetwk Stats daemon and adds it to
// the given node.
func RegisterEthStatsService(stack *node.Node, backend ethapi.Backend, url string) {
	if err := ethstats.New(stack, backend, backend.Engine(), url); err != nil {
		Fatalf("Failed to register the Tosnetwk Stats service: %v", err)
	}
}
...
// new format.
//
// e.g. gtos account new --keystore /tmp/mykeystore --lightkdf
//
// is equivalent after calling this method with:
//
// gtos --keystore /tmp/mykeystore --lightkdf account new
//
