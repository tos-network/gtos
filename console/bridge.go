...
func getJeth(vm *goja.Runtime) *goja.Object {
	jtos := vm.Get("jtos")
	if jtos == nil {
		panic(vm.ToValue("jtos object does not exist"))
	}
	return jtos.ToObject(vm)
}

// NewAccount is a wrapper around the personal.newAccount RPC method that uses a
// non-echoing password prompt to acquire the passphrase and executes the original
// RPC method (saved in jtos.newAccount) with it to actually execute the RPC call.
func (b *bridge) NewAccount(call jsre.Call) (goja.Value, error) {
	...
	if !callable {
		return nil, fmt.Errorf("jtos.newAccount is not callable")
	}
...
func (b *bridge) OpenWallet(call jsre.Call) (goja.Value, error) {
	...
	if !callable {
		return nil, fmt.Errorf("jtos.openWallet is not callable")
	}
...
// original RPC method (saved in jtos.unlockAccount) with it to actually execute
// the RPC call.
func (b *bridge) UnlockAccount(call jsre.Call) (goja.Value, error) {
	...
	if !callable {
		return nil, fmt.Errorf("jtos.unlockAccount is not callable")
	}
...
// jtos.sign) with it to actually execute the RPC call.
func (b *bridge) Sign(call jsre.Call) (goja.Value, error) {
	...
	if !callable {
		return nil, fmt.Errorf("jtos.sign is not callable")
	}
...
func (b *bridge) SleepBlocks(call jsre.Call) (goja.Value, error) {
	...
	if err := b.client.Call(&lastNumber, "tos_blockNumber"); err != nil {
		return nil, err
	}
	for time.Now().Before(deadline) {
		var number hexutil.Uint64
		if err := b.client.Call(&number, "tos_blockNumber"); err != nil {
			return nil, err
		}
