...
func TestKeyStoreSignDPoSHashRejectsSecp256k1(t *testing.T) {
	dir := t.TempDir()
	ks := NewKeyStore(dir, veryLightScryptN, veryLightScryptP)
	passphrase := "passphrase"
	...
	if _, err := ks.SignDPoSHash(acc, digest); err != ErrUnsupportedSigningKey {
		t.Fatalf("SignDPoSHash: have %v want %v", err, ErrUnsupportedSigningKey)
	}

	wallet := ks.Wallets()[0]
	if _, err := wallet.SignData(accounts.Account{Address: acc.Address}, accounts.MimetypeDPoS, digest); err != ErrUnsupportedSigningKey {
		t.Fatalf("wallet.SignData(DPoS): have %v want %v", err, ErrUnsupportedSigningKey)
	}
}
