...
func TestSimulatedBackend(t *testing.T) {
	...
	if err != tosnetwk.NotFound {
		t.Fatalf("err should be `tosnetwk.NotFound` but received %v", err)
	}
...
func TestNewSimulatedBackend_AdjustTimeFail(t *testing.T) {
	...
	sim := simTestBackend(testAddr)
	// Create tx and send
	tx := types.NewTransaction(0, testAddr, big.NewInt(1000), 2100, big.NewInt(1), nil)
	...
	tx2 := types.NewTransaction(1, testAddr, big.NewInt(1000), 2100, big.NewInt(1), nil)
...
func TestSimulatedBackend_NonceAt(t *testing.T) {
	...
	tx := types.NewTransaction(nonce, testAddr, big.NewInt(1000), 2100, big.NewInt(1), nil)
	signedTx, err := types.SignTx(tx, types.HomesteadSigner{}, testKey)
	if err != nil {
		t.Errorf("could not sign tx: %v", err)
	}
...
func TestSimulatedBackend_SendTransaction(t *testing.T) {
	...
	// create a signed transaction to send
	tx := types.NewTransaction(uint64(0), testAddr, big.NewInt(1000), 2100, big.NewInt(1), nil)
	signedTx, err := types.SignTx(tx, types.HomesteadSigner{}, testKey)
	if err != nil {
		t.Errorf("could not sign tx: %v", err)
	}
...
func TestSimulatedBackend_EstimateGas(t *testing.T) {
	...
	var cases = []struct {
		name        string
		message     tosnetwk.CallMsg
		expect      uint64
		expectError error
		expectData  interface{}
	}{
		{"plain transfer(valid)", tosnetwk.CallMsg{
			From:     addr,
			To:       &addr,
			Gas:      0,
...
func TestSimulatedBackend_EstimateGasWithPrice(t *testing.T) {
	...
	var cases = []struct {
		...
		{"EstimateWithoutPrice", tosnetwk.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
			GasPrice: big.NewInt(0),
		...
		{"EstimateWithPrice", tosnetwk.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
		...
		{"EstimateWithVeryHighPrice", tosnetwk.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
		...
		{"EstimateWithSuperhighPrice", tosnetwk.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
			...
			Value:    big.NewInt(1000),
			Data:     nil,
		}, 0, nil}, // 10999=(2.2ether-1000wei)/(2e14)
	}
	for _, c := range cases {
		got, err := sim.EstimateGas(context.Background(), c.message)
		if c.expectError != nil {
...
func TestSimulatedBackend_PendingNonceAt(t *testing.T) {
	...
	tx = types.NewTransaction(uint64(1), testAddr, big.NewInt(1000), 2100, big.NewInt(1), nil)
	signedTx, err = types.SignTx(tx, types.HomesteadSigner{}, testKey)
	if err != nil {
		t.Errorf("could not sign tx: %v", err)
	}
...
func TestSimulatedBackend_PendingAndCallContract(t *testing.T) {
	...
	res, err := sim.PendingCallContract(bgCtx, tosnetwk.CallMsg{
		From: testAddr,
		To:   &addr,
		Data: input,
	})
	...
	res, err = sim.CallContract(bgCtx, tosnetwk.CallMsg{
		From: testAddr,
		To:   &addr,
		Data: input,
	}, nil)
...
func TestSimulatedBackend_CallContractRevert(t *testing.T) {
	...
	call[0] = func(input []byte) ([]byte, error) {
		return sim.PendingCallContract(bgCtx, tosnetwk.CallMsg{
			From: testAddr,
			To:   &addr,
			Data: input,
		})
	...
	call[1] = func(input []byte) ([]byte, error) {
		return sim.CallContract(bgCtx, tosnetwk.CallMsg{
			From: testAddr,
			To:   &addr,
			Data: input,
		}, nil)
