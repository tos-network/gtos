...
func (tosNode *TOS) StateAtBlock(block *types.Block, reexec uint64, base *state.StateDB, checkLive bool, preferDisk bool) (statedb *state.StateDB, err error) {
	var (
		current  *types.Block
	...
	if checkLive {
		statedb, err = tosNode.blockchain.StateAt(block.Root())
		if err == nil {
			return statedb, nil
		}
	}
	if base != nil {
		if preferDisk {
			...
			database = state.NewDatabaseWithConfig(tosNode.chainDb, &trie.Config{Cache: 16})
		...
		current = tosNode.blockchain.GetBlock(block.ParentHash(), block.NumberU64()-1)
	} else {
		...
		for i := uint64(0); i < reexec; i++ {
			...
			parent := tosNode.blockchain.GetBlock(current.ParentHash(), current.NumberU64()-1)
			if parent == nil {
