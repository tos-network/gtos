...
func newTestBackend(t *testing.T, londonBlock *big.Int, pending bool) *testBackend {
	...
	blocks, _ := core.GenerateChain(gspec.Config, genesis, engine, db, testHead+1, func(i int, b *core.BlockGen) {
		...
		_ = big.NewInt(int64(i+1) * params.GWei) // gas price is fixed in GTOS, not a field
...
func TestSuggestTipCap(t *testing.T) {
	...
	// In GTOS, gas price is fixed at GTOSPriceWei regardless of block content.
	fixedPrice := params.GTOSPrice()
	var cases = []struct {
		fork   *big.Int // London fork number
		expect *big.Int // Expected gasprice suggestion
	}{
		{nil, fixedPrice},
		{big.NewInt(0), fixedPrice}, // Fork point in genesis
		{big.NewInt(1), fixedPrice}, // Fork point in first block
		{big.NewInt(32), fixedPrice}, // Fork point in last block
		{big.NewInt(33), fixedPrice}, // Fork point in the future
	}
	for _, c := range cases {
