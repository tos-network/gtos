...
var (
	...
	maxQueuedHeaders            = 32 * 1024                         // [tos/62] Maximum number of headers to queue for import (DOS protection)
...
type Downloader struct {
	...
	// Skeleton sync
	skeleton *skeleton // Header skeleton to backfill the chain with (tos2 mode)
...
func New(checkpoint uint64, stateDb tosdb.Database, mux *event.TypeMux, chain BlockChain, lightchain LightChain, dropPeer peerDropFn, success func()) *Downloader {
	if lightchain == nil {
		lightchain = chain
	}
	dl := &Downloader{
...
// these are zero.
func (d *Downloader) Progress() gtos.SyncProgress {
