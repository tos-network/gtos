...
func TestTraceCall(t *testing.T) {
	...
	var testSuite = []struct {
		...
		},
		{
			...
			config: &TraceCallConfig{
				BlockOverrides: &tosapi.BlockOverrides{Number: (*hexutil.Big)(big.NewInt(0x1337))},
			},
			expectErr: nil,
...
func TestTracingWithOverrides(t *testing.T) {
	...
	var testSuite = []struct {
		...
		// Call which can only succeed if state is state overridden
		{
			...
			config: &TraceCallConfig{
				StateOverrides: &tosapi.StateOverride{
					randomAccounts[0].addr: tosapi.OverrideAccount{Balance: newRPCBalance(new(big.Int).Mul(big.NewInt(1), big.NewInt(params.Ether)))},
				},
			},
		...
		//  }
		{
			...
			config: &TraceCallConfig{
				//Tracer: &tracer,
				StateOverrides: &tosapi.StateOverride{
					randomAccounts[2].addr: tosapi.OverrideAccount{
