...
// information related to the TOS protocol such als blocks, transactions and logs.
...
// `tos_getFilterChanges` polling method that is also used for log filters.
...
// It is part of the filter package since polling goes with tos_getFilterChanges.
...
func (api *FilterAPI) Logs(ctx context.Context, crit FilterCriteria) (*rpc.Subscription, error) {
	...
	logsSub, err := api.events.SubscribeLogs(gtos.FilterQuery(crit), matchedLogs)
	if err != nil {
		return nil, err
	}
...
// Same as gtos.FilterQuery but with UnmarshalJSON() method.
type FilterCriteria gtos.FilterQuery
...
func (api *FilterAPI) NewFilter(crit FilterCriteria) (rpc.ID, error) {
	...
	logsSub, err := api.events.SubscribeLogs(gtos.FilterQuery(crit), logs)
	if err != nil {
		return "", err
	}
