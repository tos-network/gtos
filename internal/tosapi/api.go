...
func (s *TOSAPI) MaxPriorityFeePerGas(ctx context.Context) (*hexutil.Big, error) {
...
func (s *TOSAPI) FeeHistory(ctx context.Context, blockCount rpc.DecimalOrHex, lastBlock rpc.BlockNumber, rewardPercentiles []float64) (*feeHistoryResult, error) {
...
func (s *TOSAPI) Syncing() (interface{}, error) {
...
func (s *TxPoolAPI) Inspect() map[string]map[string]map[string]string {
	...
	var format = func(tx *types.Transaction) string {
		if to := tx.To(); to != nil {
			return fmt.Sprintf("%s: %v wei + %v gas × %v wei", tx.To().Hex(), tx.Value(), tx.Gas(), tx.TxPrice())
		}
		return fmt.Sprintf("contract creation: %v wei + %v gas × %v wei", tx.Value(), tx.Gas(), tx.TxPrice())
	}
	// Flatten the pending transactions
	for account, txs := range pending {
...
func (s *PersonalAccountAPI) SignTransaction(ctx context.Context, args TransactionArgs, passwd string) (*SignTransactionResult, error) {
	...
	tx := args.toTransaction()
	if err := checkTxFee(tx.TxPrice(), tx.Gas(), s.b.RPCTxFeeCap()); err != nil {
		return nil, err
	}
	signed, err := s.signTransaction(ctx, &args, passwd)
	if err != nil {
...
//
// https://github.com/tos-network/gtos/wiki/Management-APIs#personal_sign
...
//
// https://github.com/tos-network/gtos/wiki/Management-APIs#personal_ecRecover
...
func newRPCPendingTransaction(tx *types.Transaction, current *types.Header, config *params.ChainConfig) *RPCTransaction {
	...
	if current != nil {
		if current.BaseFee != nil {
			baseFee = new(big.Int).Set(current.BaseFee)
		}
...
func (s *TransactionAPI) GetTransactionReceipt(ctx context.Context, hash common.Hash) (map[string]interface{}, error) {
	...
	// Assign the effective tx price paid.
	header, err := s.b.HeaderByHash(ctx, blockHash)
	if err != nil {
		return nil, err
	}
...
func SubmitTransaction(ctx context.Context, b Backend, tx *types.Transaction) (common.Hash, error) {
	...
	if err := checkTxFee(tx.TxPrice(), tx.Gas(), b.RPCTxFeeCap()); err != nil {
		return common.Hash{}, err
	}
...
func checkTxFee(txPrice *big.Int, gas uint64, cap float64) error {
	// Short circuit if there is no cap for transaction fee at all.
	if cap == 0 {
		return nil
	}
	feeTos := new(big.Float).Quo(new(big.Float).SetInt(new(big.Int).Mul(txPrice, new(big.Int).SetUint64(gas))), new(big.Float).SetInt(big.NewInt(params.TOS)))
...
// UnoBalance decrypts the UNO ElGamal balance for an already-unlocked account.
// The account must first be unlocked via personal_unlockAccount.
// maxAmount bounds the ECDLP search (default 1_000_000_000 TOS).
func (s *TOSAPI) UnoBalance(
	ctx context.Context,
	address common.Address,
	maxAmount *hexutil.Uint64,
	blockNrOrHash *rpc.BlockNumberOrHash,
	...
	if s == nil || s.b == nil {
		return nil, newRPCNotImplementedError("tos_unoBalance")
	}
	resolved := resolveBlockArg(blockNrOrHash)
	if err := enforceHistoryRetentionByBlockArg(s.b, resolved); err != nil {
		return nil, err
	...
	privKey, err := ks.ElgamalPrivateKey(accounts.Account{Address: address})
	if err != nil {
		return nil, &rpcAPIError{code: rpcErrPermissionDenied, message: err.Error()}
	}
	effectiveMax := uint64(1_000_000_000)
	if maxAmount != nil {
		effectiveMax = uint64(*maxAmount)
