...
import (
	...
	"github.com/tos-network/gtos/crypto/signify"
	"github.com/tos-network/gtos/internal/build"
...
var (
	...
	debExecutables = []debExecutable{
		{
			BinaryName:  "abigen",
			Description: "Source code generator to convert Tosnetwk contract definitions into easy to use, compile-time type-safe Go packages.",
		},
		{
			BinaryName:  "bootnode",
			Description: "Tosnetwk bootnode.",
		},
		{
			BinaryName:  "evm",
		...
		{
			BinaryName:  "gtos",
			Description: "Tosnetwk CLI client.",
		},
		{
			BinaryName:  "puppeth",
			Description: "Tosnetwk private network manager.",
		},
		{
			BinaryName:  "rlpdump",
		...
		{
			BinaryName:  "clef",
			Description: "Tosnetwk account management tool.",
		},
	}

	// A debian package is created for all executables listed here.

	debTosnetwk = debPackage{
		Name:        "tosnetwk",
...
func doInstall(cmdline []string) {
	...
	if !strings.Contains(runtime.Version(), "devel") {
		...
		if minor < 13 {
			...
			log.Println("tos requires at least Go version 1.13 and cannot")
...
// Running The Test Packge
//
// usage: make testpackage PACKAGE=./console

func doTestPackage(cmdline []string) {
	coverage := flag.Bool("coverage", false, "Whether to record code coverage")
	verbose := flag.Bool("v", false, "Whether to log verbosely")
	flag.CommandLine.Parse(cmdline)
	env := build.Env()
...
func doArchive(cmdline []string) {
	...
	for _, archive := range []string{gtos, alltools} {
...
func archiveUpload(archive string, blobstore string, signer string, signifyVar string) error {
	...
	if signifyVar != "" {
		...
		untrustedComment := "verify with gtos-release.pub"
...
func doDebianSource(cmdline []string) {
	var (
		...
		upload   = flag.String("upload", "", `Where to upload the source package (usually "tosnetwk/tosnetwk")`)
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, pkg := range debPackages {
		for distro, goboot := range debDistroGoBoots {
			// Prepare the debian package with the tos sources.
...
func downloadGo(goarch, goos, cachedir string) string {
	...
	godir := filepath.Join(ucache, fmt.Sprintf("gtos-go-%s-%s-%s", dlgoVersion, goos, goarch))
...
func makeWorkdir(wdflag string) string {
	...
	if wdflag != "" {
		...
		wdflag, err = ioutil.TempDir("", "gtos-build-")
	}
	if err != nil {
		log.Fatal(err)
	}
...
type debPackage struct {
	Name        string          // the name of the Debian package to produce, e.g. "tosnetwk"
...
type debMetadata struct {
	...
	// tos version being built. Note that this
...
func newDebMetadata(distro, goboot, author string, env build.Environment, t time.Time, name string, version string, exes []debExecutable) debMetadata {
	if author == "" {
		...
		author = "Tosnetwk Builds <fjl@tos.network>"
	}
	return debMetadata{
...
func doWindowsInstaller(cmdline []string) {
	...
	*workdir = makeWorkdir(*workdir)
	...
	installer, _ := filepath.Abs("gtos-" + archiveBasename(*arch, params.ArchiveVersion(env.Commit)) + ".exe")
	build.MustRunCommand("makensis.exe",
		...
		"/DARCH="+*arch,
		filepath.Join(*workdir, "gtos.nsi"),
	)
...
func doAndroidArchive(cmdline []string) {
	...
	build.MustRun(gomobileTool("bind", "-ldflags", "-s -w", "--target", "android", "--javapkg", "org.tosnetwk", "-v", "github.com/tos-network/gtos/mobile"))

	if *local {
		// If we're building locally, copy bundle to build dir and skip Maven
		os.Rename("gtos.aar", filepath.Join(GOBIN, "gtos.aar"))
		os.Rename("gtos-sources.jar", filepath.Join(GOBIN, "gtos-sources.jar"))
		return
	}
	...
	archive := "gtos-" + archiveBasename("android", params.ArchiveVersion(env.Commit)) + ".aar"
	os.Rename("gtos.aar", archive)
...
func doXCodeFramework(cmdline []string) {
	...
	bind := gomobileTool("bind", "-ldflags", "-s -w", "--target", "ios", "-v", "github.com/tos-network/gtos/mobile")
	...
	archive := "gtos-" + archiveBasename("ios", params.ArchiveVersion(env.Commit))
