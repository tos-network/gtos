...
type CallMsg struct {
	...
	AccessList types.AccessList // transaction access list.
}

// A ContractCaller provides contract calls, essentially transactions that are executed by
// the TVM but not mined into the blockchain. ContractCall is a low-level method to
...
type FilterQuery struct {
	BlockHash *common.Hash     // used by tos_getLogs, return logs only from block with this hash
