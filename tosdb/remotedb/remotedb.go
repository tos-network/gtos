...
// Package remotedb implements the key-value database layer based on a remote gtos
// node. Under the hood, it utilises the `debug_dbGet` method to implement a
// read-only database.
// There really are no guarantees in this database, since the local gtos does not
...
func (db *Database) ReadAncients(fn func(op tosdb.AncientReaderOp) error) (err error) {
	return fn(db)
}
...
func (db *Database) ModifyAncients(f func(tosdb.AncientWriteOp) error) (int64, error) {
	panic("not supported")
}
...
func (db *Database) NewBatchWithSize(size int) tosdb.Batch {
	panic("not supported")
}

func (db *Database) NewIterator(prefix []byte, start []byte) tosdb.Iterator {
	panic("not supported")
}
...
func (db *Database) NewSnapshot() (tosdb.Snapshot, error) {
	panic("not supported")
}
...
func New(endpoint string) (tosdb.Database, error) {
