...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Gtos.LightScryptN, Gtos.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.tosnetwk", new NodeConfig());
			node.start();
			...
			TosnetwkClient ec = node.getTosnetwkClient();
	...
	// signatures too. Regression test for tos issue #14599.
...
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altogtoser
	...
	workspace, err := ioutil.TempDir("", "gtos-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/tosnetwk/gtostest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Gtos and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.tosnetwk", "github.com/tos-network/gtos/mobile")
	...
	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "tosnetwk", "gtostest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
