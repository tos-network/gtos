...
- [x] 新增 `tos/bft_bridge_test.go::TestApplyQCFinalitySequentialHeights`，覆盖连续 128 高度 finality 推进与防回退
...
- [ ] `~/tos` Engine API 仍为最小骨架：`GetPayload` 目前返回占位的空交易列表 payload（RLP `0xc0`），`NewPayload` 已接入最小 RLP 列表结构校验（含内部 item 边界校验，非法 payload 返回 `valid=false`）但尚未接入真实执行校验
