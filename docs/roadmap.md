...
> - 关键未完成项：`~/tos` Engine API 目前仍是“最小语义”（空 payload/基础校验）；`forkchoiceUpdated` 已可按 `finalized_hash` 推进 stable 指针，但尚未接入真实 payload 构建与执行校验路径，双端闭环联调（3 节点 100+ finalized）尚未完成。
