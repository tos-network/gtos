...
> - `~/gtos`（`main`）：已完成 Engine API 客户端接线、提议/导入路径调用、BFT vote/QC 骨架与网络桥接、QC 触发 safe/finalized 推进、`tosalign` 签名地址库引入；并在 proposer 路径补充 `payload_commitment` 一致性校验（含 fallback 分支回归）。
