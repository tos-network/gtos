...
> - `~/gtos`（`main`）：已完成第 1/2/3/4 批仓库瘦身，删除与共识主路径无关的独立工具与旧轻客户端模块（`cmd/{clef,devp2p,faucet,p2psim,rlpdump,abidump}`、`les`、`tosstats`、`mobile`、`light`）；`snap` 已内聚 `NodeSet/NodeList` 到本包，不再依赖 `light` 包；`cmd/gtos` 已移除 light/ethstats CLI 入口接线。
