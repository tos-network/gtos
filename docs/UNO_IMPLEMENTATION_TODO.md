...
- [/] Explicit parity tests partially done (added txpool-vs-execution parity tests for invalid-envelope/unsupported-action, nonce-too-low, low-gas, nonzero-value, shield insufficient-balance, oversized-proof-bundle, sender signer missing/type-mismatch, transfer receiver version-overflow, unshield sender version-overflow, transfer receiver missing-signer, and shield/transfer/unshield invalid-proof-shape; broader accept/reject matrix still pending).
