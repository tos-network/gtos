...
import (
	...
	"github.com/tos-network/gtos/crypto/ecies"
	"github.com/tos-network/gtos/p2p/simulations/pipes"
...
var protocol8HandshakeAuthTests = []handshakeAuthTest{
	// (Auth₂) Protocol-8 encoding
	{
		input: `
	...
	// (Auth₃) RLPx v4 Protocol-8 encoding with version 56, additional list elements
	{
		input: `
...
var protocol8HandshakeRespTests = []handshakeAckTest{
	// (Ack₂) Protocol-8 encoding
	{
		input: `
	...
	// (Ack₃) Protocol-8 encoding with version 57, additional list elements
	{
		input: `
...
func TestHandshakeForwardCompatibility(t *testing.T) {
	...
	for _, test := range protocol8HandshakeAuthTests {
	...
	for _, test := range protocol8HandshakeRespTests {
		var h handshakeState
	...
	var (
		...
		authCiphertext     = unhex(protocol8HandshakeAuthTests[0].input)
		authRespCiphertext = unhex(protocol8HandshakeRespTests[0].input)
		authMsg            = makeAuth(protocol8HandshakeAuthTests[0])
...
func BenchmarkHandshakeRead(b *testing.B) {
	var input = unhex(protocol8HandshakeAuthTests[0].input)
