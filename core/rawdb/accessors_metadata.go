...
func ReadDatabaseVersion(db tosdb.KeyValueReader) *uint64 {
	var version uint64
...
func WriteDatabaseVersion(db tosdb.KeyValueWriter, version uint64) {
...
func ReadChainConfig(db tosdb.KeyValueReader, hash common.Hash) *params.ChainConfig {
...
func WriteChainConfig(db tosdb.KeyValueWriter, hash common.Hash, cfg *params.ChainConfig) {
	if cfg == nil {
		return
	}
...
// given genesis hash.
func ReadGenesisStateSpec(db tosdb.KeyValueReader, hash common.Hash) []byte {
	data, _ := db.Get(genesisStateSpecKey(hash))
	return data
}

// WriteGenesisStateSpec writes the genesis state specification into the disk.
func WriteGenesisStateSpec(db tosdb.KeyValueWriter, hash common.Hash, data []byte) {
...
func PushUncleanShutdownMarker(db tosdb.KeyValueStore) ([]uint64, uint64, error) {
...
func PopUncleanShutdownMarker(db tosdb.KeyValueStore) {
	var uncleanShutdowns crashList
	// Read old data
	if data, err := db.Get(uncleanShutdownKey); err != nil {
		log.Warn("Error reading unclean shutdown markers", "error", err)
...
func UpdateUncleanShutdownMarker(db tosdb.KeyValueStore) {
	var uncleanShutdowns crashList
	// Read old data
	if data, err := db.Get(uncleanShutdownKey); err != nil {
		log.Warn("Error reading unclean shutdown markers", "error", err)
