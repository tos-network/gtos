...
func TestLookupStorage(t *testing.T) {
	tests := []struct {
		name                        string
		writeTxLookupEntriesByBlock func(tosdb.Writer, *types.Block)
	}{
		{
			"DatabaseV6",
			func(db tosdb.Writer, block *types.Block) {
...
func TestDeleteBloomBits(t *testing.T) {
	...
	db := NewMemoryDatabase()
	otherGenesis := common.HexToHash("0x01")
	for i := uint(0); i < 2; i++ {
		for s := uint64(0); s < 2; s++ {
			WriteBloomBits(db, i, s, params.MainnetGenesisHash, []byte{0x01, 0x02})
			WriteBloomBits(db, i, s, otherGenesis, []byte{0x01, 0x02})
		}
	}
