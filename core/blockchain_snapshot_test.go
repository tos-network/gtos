...
// (i)   Gtos restarts normally with valid legacy snapshot
// (ii)  Gtos restarts normally with valid new-format snapshot
// (iii) Gtos restarts after the crash, with broken legacy snapshot
// (iv)  Gtos restarts after the crash, with broken new-format snapshot
// (v)   Gtos restarts normally, but it's requested to be rewound to a lower point via SetHead
// (vi)  Gtos restarts normally with a stale snapshot
type snapshotTest struct {
	legacy       bool   // Flag whether the loaded snapshot is in legacy format
	crash        bool   // Flag whether the Gtos restarts from the previous crash
...
// Tests a Gtos restart with valid snapshot. Before the shutdown, all snapshot
...
// Tests a Gtos restart with valid but "legacy" snapshot. Before the shutdown,
...
// Tests a Gtos was crashed and restarts with a broken snapshot. In this case the
...
// Tests a Gtos was crashed and restarts with a broken snapshot. In this case
...
// Tests a Gtos was crashed and restarts with a broken and "legacy format"
...
// Tests a Gtos was running with snapshot enabled. Then restarts without
...
// Tests a Gtos was running with leagcy snapshot enabled. Then restarts
...
// Tests the Gtos was running with snapshot enabled and resetHead is applied.
...
// Tests the Gtos was running with snapshot(legacy-format) enabled and resetHead
...
// Tests the Gtos was running with snapshot(legacy-format) enabled and upgrades
// the disk layer journal(journal generator) to latest format. After that the Gtos
// is restarted from a crash. In this case Gtos will find the new-format disk layer
