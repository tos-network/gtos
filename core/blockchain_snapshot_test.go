...
func (basic *snapshotTestBasic) prepare(t *testing.T) (*BlockChain, []*types.Block) {
	...
	chain, err := NewBlockChain(db, cacheConfig, params.AllDPoSProtocolChanges, engine, nil, nil)
	if err != nil {
...
// It can be used for testing that restart GTOS normally.
...
func (snaptest *snapshotTest) test(t *testing.T) {
	...
	chain.Stop()
	newchain, err := NewBlockChain(snaptest.db, nil, params.AllDPoSProtocolChanges, snaptest.engine, nil, nil)
	if err != nil {
...
// It can be used for testing that restart GTOS after the crash.
...
func (snaptest *crashSnapshotTest) test(t *testing.T) {
	...
	newchain, err := NewBlockChain(newdb, nil, params.AllDPoSProtocolChanges, snaptest.engine, nil, nil)
	if err != nil {
		t.Fatalf("Failed to recreate chain: %v", err)
	}
	newchain.Stop()
...
func (snaptest *gappedSnapshotTest) test(t *testing.T) {
	...
	newchain, err := NewBlockChain(snaptest.db, cacheConfig, params.AllDPoSProtocolChanges, snaptest.engine, nil, nil)
	if err != nil {
		t.Fatalf("Failed to recreate chain: %v", err)
	}
...
func (snaptest *wipeCrashSnapshotTest) test(t *testing.T) {
	...
	newchain, err := NewBlockChain(snaptest.db, config, params.AllDPoSProtocolChanges, snaptest.engine, nil, nil)
	if err != nil {
		t.Fatalf("Failed to recreate chain: %v", err)
	}
	...
	_, err = NewBlockChain(snaptest.db, config, params.AllDPoSProtocolChanges, snaptest.engine, nil, nil)
	if err != nil {
		t.Fatalf("Failed to recreate chain: %v", err)
	}
...
// Tests the GTOS was running with a complete snapshot and then imports a few
