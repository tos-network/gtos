...
func testReorgLong(t *testing.T, full bool) {
	testReorg(t, []int64{0, 0, -9}, []int64{0, 0, 0, -9}, 393856, full)
}
...
func TestFastVsFullChains(t *testing.T) {
	...
	blocks, receipts := GenerateChain(gspec.Config, genesis, ethash.NewFaker(), gendb, 1024, func(i int, block *BlockGen) {
		...
		if i%3 == 2 {
			for j := 0; j < i%4+1; j++ {
				tx, err := types.SignTx(types.NewTransaction(block.TxNonce(address), common.Address{0x00}, big.NewInt(1000), 2100, nil, nil), signer, key)
				if err != nil {
					panic(err)
				}
				block.AddTx(tx)
...
func TestChainTxReorgs(t *testing.T) {
	...
	postponed, _ := types.SignTx(types.NewTransaction(0, addr1, big.NewInt(1000), 2100, nil, nil), signer, key1)
