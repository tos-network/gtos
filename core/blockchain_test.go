...
func newCanonical(engine consensus.Engine, n int, full bool) (tosdb.Database, *BlockChain, error) {
	...
	blockchain, _ := NewBlockChain(db, nil, params.TestChainConfig, engine, nil, nil)
...
// Tests that reorganising a longer chain after a shorter one overwrites the
// canonical numbers and links in the database.
// With DPoS constant difficulty (diffNoTurn=1), total difficulty is simply
// genesis_D + N where N is the chain length. A longer chain always wins.
func TestReorgLongHeaders(t *testing.T) { testReorgLong(t, false) }
func TestReorgLongBlocks(t *testing.T)  { testReorgLong(t, true) }

func testReorgLong(t *testing.T, full bool) {
	// first=3 blocks, second=4 blocks; second has higher TD and should become canonical.
	// Expected TD contribution from second: 4 * diffNoTurn(1) = 4.
	testReorg(t, []int64{0, 0, 0}, []int64{0, 0, 0, 0}, 4, full)
}

// Tests that reorganising a chain with more blocks after one with fewer blocks
// overwrites the canonical numbers and links in the database.
// With DPoS constant difficulty the longer chain always carries more total difficulty.
func TestReorgShortHeaders(t *testing.T) { testReorgShort(t, false) }
func TestReorgShortBlocks(t *testing.T)  { testReorgShort(t, true) }

func testReorgShort(t *testing.T, full bool) {
	// first=3 blocks, second=8 blocks; second wins by having more TD.
	// Expected TD contribution from second: 8 * diffNoTurn(1) = 8.
	first := make([]int64, 3)
	second := make([]int64, 8)
	testReorg(t, first, second, 8, full)
}
...
func testReorgBadHashes(t *testing.T, full bool) {
	...
	ncm, err := NewBlockChain(blockchain.db, nil, blockchain.chainConfig, dpos.NewFaker(), nil, nil)
	if err != nil {
...
func TestFastVsFullChains(t *testing.T) {
	...
	blocks, receipts := GenerateChain(gspec.Config, genesis, dpos.NewFaker(), gendb, 1024, func(i int, block *BlockGen) {
		...
		if i%3 == 2 {
			for j := 0; j < i%4+1; j++ {
				tx, err := signTestSignerTx(signer, key, block.TxNonce(address), common.Address{0x00}, big.NewInt(1000), params.TxGas, block.header.BaseFee, nil)
				if err != nil {
					panic(err)
				}
				block.AddTx(tx)
	...
	archive, _ := NewBlockChain(archiveDb, nil, gspec.Config, dpos.NewFaker(), nil, nil)
	defer archive.Stop()
	...
	fast, _ := NewBlockChain(fastDb, nil, gspec.Config, dpos.NewFaker(), nil, nil)
	defer fast.Stop()
	...
	ancient, _ := NewBlockChain(ancientDb, nil, gspec.Config, dpos.NewFaker(), nil, nil)
	defer ancient.Stop()
...
func TestLightVsFastVsFullChainHeads(t *testing.T) {
	...
	var (
		...
		gspec   = &Genesis{
			Config: params.TestChainConfig, ExtraData: testDPoSGenesisExtra(),
			Alloc:   GenesisAlloc{address: {Balance: funds}},
			BaseFee: big.NewInt(params.InitialBaseFee),
		}
		genesis = gspec.MustCommit(gendb)
	...
	archive, _ := NewBlockChain(archiveDb, &archiveCaching, gspec.Config, dpos.NewFaker(), nil, nil)
	if n, err := archive.InsertChain(blocks); err != nil {
		t.Fatalf("failed to process block %d: %v", n, err)
	}
	defer archive.Stop()
	...
	lightDb := makeDb()
	defer lightDb.Close()
	light, _ := NewBlockChain(lightDb, nil, gspec.Config, dpos.NewFaker(), nil, nil)
...
func TestChainTxReorgs(t *testing.T) {
	...
	postponed, _ := signTestSignerTx(signer, key1, 0, addr1, big.NewInt(1000), params.TxGas, big.NewInt(params.InitialBaseFee), nil)
	swapped, _ := signTestSignerTx(signer, key1, 1, addr1, big.NewInt(1000), params.TxGas, big.NewInt(params.InitialBaseFee), nil)
	...
	chain, _ := GenerateChain(gspec.Config, genesis, dpos.NewFaker(), db, 3, func(i int, gen *BlockGen) {
		switch i {
		case 0:
			pastDrop, _ = signTestSignerTx(signer, key2, gen.TxNonce(addr2), addr2, big.NewInt(1000), params.TxGas, gen.header.BaseFee, nil)

			gen.AddTx(pastDrop)  // This transaction will be dropped in the fork from below the split point
			gen.AddTx(postponed) // This transaction will be postponed till block #3 in the fork

		case 2:
			freshDrop, _ = signTestSignerTx(signer, key2, gen.TxNonce(addr2), addr2, big.NewInt(1000), params.TxGas, gen.header.BaseFee, nil)
	...
	chain, _ = GenerateChain(gspec.Config, genesis, dpos.NewFaker(), db, 5, func(i int, gen *BlockGen) {
		switch i {
		case 0:
			pastAdd, _ = signTestSignerTx(signer, key3, gen.TxNonce(addr3), addr3, big.NewInt(1000), params.TxGas, gen.header.BaseFee, nil)
		...
		case 2:
			...
			freshAdd, _ = signTestSignerTx(signer, key3, gen.TxNonce(addr3), addr3, big.NewInt(1000), params.TxGas, gen.header.BaseFee, nil)
			gen.AddTx(freshAdd) // This transaction will be added exactly at reorg time

		case 3:
			futureAdd, _ = signTestSignerTx(signer, key3, gen.TxNonce(addr3), addr3, big.NewInt(1000), params.TxGas, gen.header.BaseFee, nil)
...
func TestReorgSideEvent(t *testing.T) {
	...
	replacementBlocks, _ := GenerateChain(gspec.Config, genesis, dpos.NewFaker(), db, 4, func(i int, gen *BlockGen) {
		tx, err := signTestSignerTx(signer, key1, gen.TxNonce(addr1), addr1, new(big.Int), 1000000, gen.header.BaseFee, nil)
		if i == 2 {
			gen.OffsetTime(-9)
		}
		if err != nil {
	...
	// With DPoS constant difficulty, replacementBlocks[2] ties with the chain head
	// (both at TD=3). The tie-break is non-deterministic, so rB[2] may or may not
	// fire a ChainSideEvent. rB[0] and rB[1] are always side events (TD < 3), and
	// after rB[3] triggers the reorg, chain[0..2] are always demoted to side events.
	// That gives 5 guaranteed events; rB[2] is an optional 6th.
	expectedSideHashes := map[common.Hash]bool{
		replacementBlocks[0].Hash(): true,
		replacementBlocks[1].Hash(): true,
		replacementBlocks[2].Hash(): true, // optional: only fires if rB[2] loses the equal-TD tie
		chain[0].Hash():             true,
		chain[1].Hash():             true,
		chain[2].Hash():             true,
	}
	requiredCount := 5 // rB[0], rB[1], chain[0..2] are always demoted
	...
	for {
		...
		case ev := <-chainSideCh:
			...
			// Once all mandatory events have arrived, use a short window to catch
			// the optional rB[2] side event (if the equal-TD tie-break went against it).
			if i >= requiredCount {
				timeout.Reset(250 * time.Millisecond)
			} else {
...
func TestReplayProtectionTransition(t *testing.T) {
	...
	blocks, _ := GenerateChain(gspec.Config, genesis, dpos.NewFaker(), db, 4, func(i int, block *BlockGen) {
		signer := types.LatestSigner(gspec.Config)
		tx, err := signTestSignerTx(signer, key, block.TxNonce(address), common.Address{}, new(big.Int), 21000, new(big.Int), nil)
		if err != nil {
			t.Fatal(err)
		}
		block.AddTx(tx)
	...
	blocks, _ = GenerateChain(config, blocks[len(blocks)-1], dpos.NewFaker(), db, 4, func(i int, block *BlockGen) {
		var (
			...
			basicTx = func(signer types.Signer) (*types.Transaction, error) {
				return signTestSignerTx(signer, key, block.TxNonce(address), common.Address{}, new(big.Int), 21000, new(big.Int), nil)
			}
		)
		if i == 0 {
...
func TestBlockchainHeaderchainReorgConsistency(t *testing.T) {
	...
	(&Genesis{Config: params.TestChainConfig, BaseFee: big.NewInt(params.InitialBaseFee), ExtraData: testDPoSGenesisExtra()}).MustCommit(diskdb)

	chain, err := NewBlockChain(diskdb, nil, params.TestChainConfig, engine, nil, nil)
	if err != nil {
...
func TestBlockchainRecovery(t *testing.T) {
	...
	var (
		...
		address = crypto.PubkeyToAddress(key.PublicKey)
		funds   = big.NewInt(1000000000000000)
		gspec   = &Genesis{Config: params.TestChainConfig, ExtraData: testDPoSGenesisExtra(), Alloc: GenesisAlloc{address: {Balance: funds}}}
		genesis = gspec.MustCommit(gendb)
	)
	height := uint64(1024)
	blocks, receipts := GenerateChain(gspec.Config, genesis, dpos.NewFaker(), gendb, int(height), nil)
...
func TestInsertReceiptChainRollback(t *testing.T) {
	...
	defer ancientDb.Close()
	gspec := Genesis{Config: params.AllDPoSProtocolChanges, ExtraData: testDPoSGenesisExtra()}
	gspec.MustCommit(ancientDb)
	ancientChain, _ := NewBlockChain(ancientDb, nil, gspec.Config, dpos.NewFaker(), nil, nil)
...
func testSideImport(t *testing.T, numCanonBlocksInSidechain, blocksBetweenCommonAncestorAndPruneblock int, mergePoint int) {
	...
	var (
		...
		genEngine = dpos.NewFaker()
		runEngine = dpos.NewFaker()
		db        = rawdb.NewMemoryDatabase()
	...
	chain, err := NewBlockChain(diskdb, nil, &chainConfig, runEngine, nil, nil)
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
...
func testInsertKnownChainData(t *testing.T, typ string) {
	...
	// A longer chain (65 blocks from the fork point).
	...
	// An even longer chain (66 blocks from the fork point). With DPoS constant
	// difficulty, length determines total difficulty, so this chain wins over blocks2.
	...
	(&Genesis{Config: params.TestChainConfig, BaseFee: big.NewInt(params.InitialBaseFee), ExtraData: testDPoSGenesisExtra()}).MustCommit(chaindb)
	defer chaindb.Close()

	chain, err := NewBlockChain(chaindb, nil, params.TestChainConfig, engine, nil, nil)
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
	...
	// Import the longer chain (blocks3, 66 blocks) with higher total difficulty.
	if err := inserter(append(blocks, blocks3...), append(receipts, receipts3...)); err != nil {
		t.Fatalf("failed to insert chain data: %v", err)
	}
	asserter(t, blocks3[len(blocks3)-1])

	// Import the shorter chain (blocks2, 65 blocks) with lower total difficulty.
	if err := inserter(append(blocks, blocks2...), append(receipts, receipts2...)); err != nil {
		t.Fatalf("failed to insert chain data: %v", err)
	}
	// The head shouldn't change (blocks3 has higher TD).
	asserter(t, blocks3[len(blocks3)-1])

	// Rollback the longer chain and re-insert the shorter chain again
	chain.SetHead(rollback - 1)
	if err := inserter(append(blocks, blocks2...), append(receipts, receipts2...)); err != nil {
		t.Fatalf("failed to insert chain data: %v", err)
	}
...
func testInsertKnownChainDataWithMerging(t *testing.T, typ string, mergeHeight int) {
	...
	var (
		db        = rawdb.NewMemoryDatabase()
		genesis   = (&Genesis{BaseFee: big.NewInt(params.InitialBaseFee), Config: &chainConfig, ExtraData: testDPoSGenesisExtra()}).MustCommit(db)
	...
	chain, err := NewBlockChain(chaindb, nil, &chainConfig, runEngine, nil, nil)
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
	var (
...
//
// NOTE: With DPoS constant difficulty (diffNoTurn=1), a shorter chain cannot
// win a reorg. This test is skipped; the reorg canon-cleanup code is covered
// by TestReorgLong/TestReorgShort and the set-head tests.
func TestReorgToShorterRemovesCanonMapping(t *testing.T) {
	t.Skip("DPoS has constant difficulty; shorter chains cannot win reorgs")
...
//
// NOTE: skipped for the same reason as TestReorgToShorterRemovesCanonMapping.
...
func TestTransactionIndices(t *testing.T) {
	...
	height := uint64(128)
	blocks, receipts := GenerateChain(gspec.Config, genesis, dpos.NewFaker(), gendb, int(height), func(i int, block *BlockGen) {
		tx, err := signTestSignerTx(signer, key, block.TxNonce(address), common.Address{0x00}, big.NewInt(1000), params.TxGas, block.header.BaseFee, nil)
		if err != nil {
			panic(err)
		}
	...
	blocks2, _ := GenerateChain(gspec.Config, blocks[len(blocks)-1], dpos.NewFaker(), gendb, 10, nil)
	...
	l := uint64(0)
	chain, err := NewBlockChain(ancientDb, nil, params.TestChainConfig, dpos.NewFaker(), nil, &l)
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
	headers := make([]*types.Header, len(blocks))
...
func benchmarkLargeNumberOfValueToNonexisting(b *testing.B, numTxs, numBlocks int, recipientFn func(uint64) common.Address, dataFn func(uint64) []byte) {
	...
	blockGenerator := func(i int, block *BlockGen) {
		...
		for txi := 0; txi < numTxs; txi++ {
			...
			tx, err := signTestSignerTx(signer, testBankKey, uniq, recipient, big.NewInt(1), params.TxGas, block.header.BaseFee, nil)
			if err != nil {
				b.Error(err)
			}
			block.AddTx(tx)
...
func TestSetCanonical(t *testing.T) {
	...
	canon, _ := GenerateChain(params.TestChainConfig, genesis, engine, db, 2*TriesInMemory, func(i int, gen *BlockGen) {
		tx, err := signTestSignerTx(signer, key, gen.TxNonce(address), common.Address{0x00}, big.NewInt(1000), params.TxGas, gen.header.BaseFee, nil)
		if err != nil {
			panic(err)
		}
		gen.AddTx(tx)
	...
	side, _ := GenerateChain(params.TestChainConfig, genesis, engine, db, 2*TriesInMemory, func(i int, gen *BlockGen) {
		tx, err := signTestSignerTx(signer, key, gen.TxNonce(address), common.Address{0x00}, big.NewInt(1), params.TxGas, gen.header.BaseFee, nil)
		if err != nil {
			panic(err)
		}
		gen.AddTx(tx)
