...
func (c *testChain) State() (*state.StateDB, error) {
	...
	if *c.trigger {
		...
		c.statedb.SetBalance(c.address, new(big.Int).SetUint64(params.TOS))
		*c.trigger = false
	}
	return stdb, nil
}
...
func testUNOTransferWireMalformedProof(t *testing.T, to common.Address) []byte {
	t.Helper()
	payload, err := coreuno.EncodeTransferPayload(coreuno.TransferPayload{
		To:            to,
		NewSender:     testUNOCiphertext(ristretto255.NewIdentityElement().Bytes(), ristretto255.NewIdentityElement().Bytes()),
	...
	wire, err := coreuno.EncodeEnvelope(coreuno.ActionTransfer, payload)
	if err != nil {
		t.Fatalf("EncodeEnvelope malformed transfer: %v", err)
	}
	return wire
}

func testUNOShieldWireMalformedProof(t *testing.T, amount uint64) []byte {
	t.Helper()
	payload, err := coreuno.EncodeShieldPayload(coreuno.ShieldPayload{
		Amount:      amount,
		NewSender:   testUNOCiphertext(ristretto255.NewGeneratorElement().Bytes(), ristretto255.NewIdentityElement().Bytes()),
	...
	if err != nil {
		t.Fatalf("EncodeShieldPayload malformed: %v", err)
	}
	wire, err := coreuno.EncodeEnvelope(coreuno.ActionShield, payload)
	if err != nil {
		t.Fatalf("EncodeEnvelope malformed shield: %v", err)
	}
	return wire
}

func testUNOShieldWireOversizedProof(t *testing.T, amount uint64) []byte {
	t.Helper()
	payload, err := coreuno.EncodeShieldPayload(coreuno.ShieldPayload{
		Amount:      amount,
		NewSender:   testUNOCiphertext(ristretto255.NewGeneratorElement().Bytes(), ristretto255.NewIdentityElement().Bytes()),
		ProofBundle: make([]byte, params.UNOMaxProofBytes+1),
	})
	if err != nil {
		t.Fatalf("EncodeShieldPayload oversized proof: %v", err)
	}
	wire, err := coreuno.EncodeEnvelope(coreuno.ActionShield, payload)
	if err != nil {
		t.Fatalf("EncodeEnvelope oversized shield: %v", err)
	}
	return wire
}

func testUNOTransferWireOversizedProof(t *testing.T, to common.Address) []byte {
	t.Helper()
	payload, err := coreuno.EncodeTransferPayload(coreuno.TransferPayload{
		To:            to,
		NewSender:     testUNOCiphertext(ristretto255.NewIdentityElement().Bytes(), ristretto255.NewIdentityElement().Bytes()),
	...
	if err != nil {
		t.Fatalf("EncodeTransferPayload oversized proof: %v", err)
	}
	wire, err := coreuno.EncodeEnvelope(coreuno.ActionTransfer, payload)
	if err != nil {
		t.Fatalf("EncodeEnvelope oversized transfer: %v", err)
	}
	return wire
}

func testUNOShieldWireEmptyProof(t *testing.T, amount uint64) []byte {
	t.Helper()
	payload, err := coreuno.EncodeShieldPayload(coreuno.ShieldPayload{
		Amount:      amount,
		NewSender:   testUNOCiphertext(ristretto255.NewGeneratorElement().Bytes(), ristretto255.NewIdentityElement().Bytes()),
	...
	if err != nil {
		t.Fatalf("EncodeShieldPayload empty proof: %v", err)
	}
	wire, err := coreuno.EncodeEnvelope(coreuno.ActionShield, payload)
	if err != nil {
		t.Fatalf("EncodeEnvelope empty shield proof: %v", err)
	}
	return wire
}

func testUNOUnshieldWireOversizedProof(t *testing.T, to common.Address, amount uint64) []byte {
	t.Helper()
	payload, err := coreuno.EncodeUnshieldPayload(coreuno.UnshieldPayload{
		To:          to,
		Amount:      amount,
	...
	if err != nil {
		t.Fatalf("EncodeUnshieldPayload oversized proof: %v", err)
	}
	wire, err := coreuno.EncodeEnvelope(coreuno.ActionUnshield, payload)
	if err != nil {
		t.Fatalf("EncodeEnvelope oversized unshield: %v", err)
	}
	return wire
}

func testUNOUnshieldWireMalformedProof(t *testing.T, to common.Address, amount uint64) []byte {
	t.Helper()
	payload, err := coreuno.EncodeUnshieldPayload(coreuno.UnshieldPayload{
		To:          to,
		Amount:      amount,
		NewSender:   testUNOCiphertext(ristretto255.NewIdentityElement().Bytes(), ristretto255.NewIdentityElement().Bytes()),
		ProofBundle: make([]byte, coreuno.BalanceProofSize-1),
	})
	if err != nil {
		t.Fatalf("EncodeUnshieldPayload malformed: %v", err)
	}
	wire, err := coreuno.EncodeEnvelope(coreuno.ActionUnshield, payload)
	if err != nil {
		t.Fatalf("EncodeEnvelope malformed unshield: %v", err)
	}
	return wire
}

func testUNOTransferWireEmptyProof(t *testing.T, to common.Address) []byte {
	t.Helper()
	payload, err := coreuno.EncodeTransferPayload(coreuno.TransferPayload{
		To:            to,
		NewSender:     testUNOCiphertext(ristretto255.NewIdentityElement().Bytes(), ristretto255.NewIdentityElement().Bytes()),
	...
	if err != nil {
		t.Fatalf("EncodeTransferPayload empty proof: %v", err)
	}
	wire, err := coreuno.EncodeEnvelope(coreuno.ActionTransfer, payload)
	if err != nil {
		t.Fatalf("EncodeEnvelope empty transfer proof: %v", err)
	}
	return wire
}

func testUNOUnshieldWireEmptyProof(t *testing.T, to common.Address, amount uint64) []byte {
	t.Helper()
	payload, err := coreuno.EncodeUnshieldPayload(coreuno.UnshieldPayload{
		To:          to,
		Amount:      amount,
	...
	if err != nil {
		t.Fatalf("EncodeUnshieldPayload empty proof: %v", err)
	}
	wire, err := coreuno.EncodeEnvelope(coreuno.ActionUnshield, payload)
	if err != nil {
		t.Fatalf("EncodeEnvelope empty unshield proof: %v", err)
	}
	return wire
}

func testUNOUnsupportedActionWire(t *testing.T) []byte {
	t.Helper()
	...
	if err != nil {
		t.Fatalf("rlp.EncodeToBytes unsupported action envelope: %v", err)
	}
...
func testUNOShieldWireZeroAmount(t *testing.T) []byte {
	...
	if err != nil {
		t.Fatalf("rlp.EncodeToBytes malformed shield payload: %v", err)
	}
	wire, err := coreuno.EncodeEnvelope(coreuno.ActionShield, body)
	if err != nil {
		t.Fatalf("EncodeEnvelope malformed shield payload: %v", err)
	}
	return wire
}

func testUNOTransferWireZeroReceiver(t *testing.T) []byte {
	t.Helper()
	type ciphertextRLP struct {
		Commitment []byte
		Handle     []byte
	...
	body, err := rlp.EncodeToBytes(&transferPayloadRLP{
		To: common.Address{},
		NewSender: ciphertextRLP{
			Commitment: ristretto255.NewIdentityElement().Bytes(),
			Handle:     ristretto255.NewIdentityElement().Bytes(),
		},
		ReceiverDelta: ciphertextRLP{
			Commitment: ristretto255.NewGeneratorElement().Bytes(),
	...
	if err != nil {
		t.Fatalf("EncodeEnvelope malformed transfer payload: %v", err)
	}
	return wire
}

func testUNOUnshieldWireZeroReceiver(t *testing.T) []byte {
	t.Helper()
	type ciphertextRLP struct {
		Commitment []byte
		Handle     []byte
	...
	if err != nil {
		t.Fatalf("rlp.EncodeToBytes malformed unshield payload: %v", err)
	}
	wire, err := coreuno.EncodeEnvelope(coreuno.ActionUnshield, body)
	if err != nil {
		t.Fatalf("EncodeEnvelope malformed unshield payload: %v", err)
	}
	return wire
}
...
func testUNOTransferWireSelfTransfer(t *testing.T, from common.Address) []byte {
	...
	wire, err := coreuno.EncodeEnvelope(coreuno.ActionTransfer, payload)
	if err != nil {
		t.Fatalf("EncodeEnvelope self-transfer: %v", err)
	}
	return wire
}

func testUNOUnshieldWireZeroAmount(t *testing.T) []byte {
	t.Helper()
	type ciphertextRLP struct {
		Commitment []byte
		Handle     []byte
	...
	if err != nil {
		t.Fatalf("rlp.EncodeToBytes zero-amount unshield: %v", err)
	}
	wire, err := coreuno.EncodeEnvelope(coreuno.ActionUnshield, body)
	if err != nil {
		t.Fatalf("EncodeEnvelope zero-amount unshield: %v", err)
	}
	return wire
}
...
func testUNOElgamalTx(t *testing.T, nonce uint64, gas uint64, data []byte) (*types.Transaction, common.Address, []byte) {
	return testUNOElgamalTxWithValue(t, nonce, gas, big.NewInt(0), data)
}

func testUNOElgamalTxWithValue(t *testing.T, nonce uint64, gas uint64, value *big.Int, data []byte) (*types.Transaction, common.Address, []byte) {
	t.Helper()
...
func TestUNOTxPoolExecutionRejectParityShieldSenderVersionOverflow(t *testing.T) {
	t.Parallel()

	pool, _ := setupTxPool()
	defer pool.Stop()

	tx, from, pub := testUNOElgamalTx(t, 0, 1_200_000, testUNOShieldWire(t, 1))
	testSetElgamalSigner(pool, from, pub)
	testSetUNOVersion(pool, from, math.MaxUint64)
...
func TestUNOTxPoolExecutionRejectParityTransferSenderVersionOverflow(t *testing.T) {
	t.Parallel()
	...
	tx, from, pub := testUNOElgamalTx(t, 0, 1_200_000, testUNOTransferWire(t, receiver))
	testSetElgamalSigner(pool, from, pub)
	testSetElgamalSigner(pool, receiver, receiverPub)
	testSetUNOVersion(pool, from, math.MaxUint64)
...
func TestUNOTxPoolExecutionRejectParityNonceTooLow(t *testing.T) {
	...
	tx, from, pub := testUNOElgamalTx(t, 0, 1_000_000, testUNOShieldWire(t, 1))
	testSetElgamalSigner(pool, from, pub)
	sufficient := new(big.Int).Add(tx.Cost(), new(big.Int).SetUint64(params.TOS))
	sufficient.Add(sufficient, big.NewInt(100))
...
func TestUNOTxPoolExecutionRejectParityShieldInvalidProofShape(t *testing.T) {
	...
	execState := newTTLDeterminismState(t)
	setupElgamalSigner(t, execState, from, pub)
	execState.SetBalance(from, new(big.Int).Set(minBalance))
...
func TestUNOTxPoolExecutionRejectParityTransferInvalidProofShape(t *testing.T) {
	t.Parallel()
	...
	tx, from, pub := testUNOElgamalTx(t, 0, 1_000_000, testUNOTransferWireMalformedProof(t, receiver))
	testSetElgamalSigner(pool, from, pub)
	testSetElgamalSigner(pool, receiver, receiverPub)
	testAddBalance(pool, from, big.NewInt(1_000_000))
...
func TestUNOTxPoolExecutionRejectParityUnshieldInvalidProofShape(t *testing.T) {
	t.Parallel()
	...
	tx, from, pub := testUNOElgamalTx(t, 0, 1_000_000, testUNOUnshieldWireMalformedProof(t, to, 1))
	testSetElgamalSigner(pool, from, pub)
	testAddBalance(pool, from, big.NewInt(1_000_000))
...
func TestUNOTxPoolExecutionRejectParityInvalidEnvelope(t *testing.T) {
	t.Parallel()

	pool, _ := setupTxPool()
	defer pool.Stop()

	tx, from, pub := testUNOElgamalTx(t, 0, 1_000_000, []byte("bad"))
	testSetElgamalSigner(pool, from, pub)
	testAddBalance(pool, from, big.NewInt(1_000_000))
...
func TestUNOTxPoolExecutionRejectParityUnsupportedAction(t *testing.T) {
	t.Parallel()

	pool, _ := setupTxPool()
	defer pool.Stop()

	tx, from, pub := testUNOElgamalTx(t, 0, 1_000_000, testUNOUnsupportedActionWire(t))
	testSetElgamalSigner(pool, from, pub)
	testAddBalance(pool, from, big.NewInt(1_000_000))

	txpoolErr := pool.AddRemote(tx)
	if !errors.Is(txpoolErr, coreuno.ErrUnsupportedAction) {
		t.Fatalf("txpool expected %v, got %v", coreuno.ErrUnsupportedAction, txpoolErr)
	}
...
func TestUNOTxPoolExecutionRejectParityNonZeroValue(t *testing.T) {
	t.Parallel()

	pool, _ := setupTxPool()
	defer pool.Stop()

	tx, from, pub := testUNOElgamalTxWithValue(t, 0, 1_000_000, big.NewInt(1), testUNOShieldWire(t, 1))
	testSetElgamalSigner(pool, from, pub)
	testSetBalanceExact(pool, from, new(big.Int).Add(tx.Cost(), big.NewInt(1)))
	...
	execState := newTTLDeterminismState(t)
	setupElgamalSigner(t, execState, from, pub)
	execState.SetBalance(from, new(big.Int).Add(tx.Cost(), big.NewInt(1)))
...
func TestUNOTxPoolExecutionRejectParityOversizedProofBundle(t *testing.T) {
	t.Parallel()

	pool, _ := setupTxPool()
	defer pool.Stop()

	tx, from, pub := testUNOElgamalTx(t, 0, 1_200_000, testUNOShieldWireOversizedProof(t, 1))
	testSetElgamalSigner(pool, from, pub)
	testSetBalanceExact(pool, from, new(big.Int).Add(tx.Cost(), new(big.Int).SetUint64(params.TOS)))
...
func TestUNOTxPoolExecutionRejectParityTransferOversizedProofBundle(t *testing.T) {
	t.Parallel()
	...
	tx, from, pub := testUNOElgamalTx(t, 0, 1_200_000, testUNOTransferWireOversizedProof(t, receiver))
	testSetElgamalSigner(pool, from, pub)
	testSetElgamalSigner(pool, receiver, receiverPub)
	testSetBalanceExact(pool, from, new(big.Int).Set(tx.Cost()))
...
func TestUNOTxPoolExecutionRejectParityUnshieldOversizedProofBundle(t *testing.T) {
	t.Parallel()
	...
	to := common.HexToAddress("0x9876")
	tx, from, pub := testUNOElgamalTx(t, 0, 1_200_000, testUNOUnshieldWireOversizedProof(t, to, 1))
	testSetElgamalSigner(pool, from, pub)
	testSetBalanceExact(pool, from, new(big.Int).Set(tx.Cost()))
...
func TestUNOTxPoolExecutionRejectParityLowGas(t *testing.T) {
	t.Parallel()

	pool, _ := setupTxPool()
	defer pool.Stop()

	tx, from, pub := testUNOElgamalTx(t, 0, 20_000, testUNOShieldWire(t, 1))
	testSetElgamalSigner(pool, from, pub)
	testSetBalanceExact(pool, from, new(big.Int).Add(tx.Cost(), new(big.Int).SetUint64(params.TOS)))

	txpoolErr := pool.AddRemote(tx)
	if !errors.Is(txpoolErr, ErrIntrinsicGas) {
		t.Fatalf("txpool expected %v, got %v", ErrIntrinsicGas, txpoolErr)
	}
...
func TestUNOTxPoolExecutionRejectParityShieldZeroAmount(t *testing.T) {
	t.Parallel()

	pool, _ := setupTxPool()
	defer pool.Stop()

	tx, from, pub := testUNOElgamalTx(t, 0, 1_200_000, testUNOShieldWireZeroAmount(t))
	testSetElgamalSigner(pool, from, pub)
	testSetBalanceExact(pool, from, new(big.Int).Set(tx.Cost()))
...
func TestUNOTxPoolExecutionRejectParityTransferZeroReceiver(t *testing.T) {
	t.Parallel()

	pool, _ := setupTxPool()
	defer pool.Stop()

	tx, from, pub := testUNOElgamalTx(t, 0, 1_200_000, testUNOTransferWireZeroReceiver(t))
	testSetElgamalSigner(pool, from, pub)
	testSetBalanceExact(pool, from, new(big.Int).Set(tx.Cost()))
...
func TestUNOTxPoolExecutionRejectParityUnshieldZeroReceiver(t *testing.T) {
	t.Parallel()
...
func TestUNOTxPoolExecutionRejectParityShieldEmptyProof(t *testing.T) {
	t.Parallel()

	pool, _ := setupTxPool()
	defer pool.Stop()

	tx, from, pub := testUNOElgamalTx(t, 0, 1_200_000, testUNOShieldWireEmptyProof(t, 1))
	testSetElgamalSigner(pool, from, pub)
	testSetBalanceExact(pool, from, new(big.Int).Add(tx.Cost(), new(big.Int).SetUint64(params.TOS)))
...
func TestUNOTxPoolExecutionRejectParityTransferEmptyProof(t *testing.T) {
	t.Parallel()
	...
	tx, from, pub := testUNOElgamalTx(t, 0, 1_200_000, testUNOTransferWireEmptyProof(t, receiver))
	testSetElgamalSigner(pool, from, pub)
	testSetElgamalSigner(pool, receiver, receiverPub)
	testSetBalanceExact(pool, from, new(big.Int).Set(tx.Cost()))
...
func TestUNOTxPoolExecutionRejectParityUnshieldEmptyProof(t *testing.T) {
	t.Parallel()
	...
	to := common.HexToAddress("0x9876")
	tx, from, pub := testUNOElgamalTx(t, 0, 1_200_000, testUNOUnshieldWireEmptyProof(t, to, 1))
	testSetElgamalSigner(pool, from, pub)
	testSetBalanceExact(pool, from, new(big.Int).Set(tx.Cost()))
...
func TestUNOTxPoolExecutionRejectParityTransferSelfTransfer(t *testing.T) {
	t.Parallel()

	// Generate the elgamal keypair up-front so we know `from` before building the wire.
	priv, err := accountsigner.GenerateElgamalPrivateKey(crand.Reader)
	if err != nil {
		t.Fatalf("GenerateElgamalPrivateKey: %v", err)
	}
	...
	wire := testUNOTransferWireSelfTransfer(t, from)

	routerAddr := params.PrivacyRouterAddress
	rawTx := types.NewTx(&types.SignerTx{
		ChainID:    params.TestChainConfig.ChainID,
		Nonce:      0,
		To:         &routerAddr,
		Value:      big.NewInt(0),
	...
	signer := types.LatestSignerForChainID(params.TestChainConfig.ChainID)
	sig, err := accountsigner.SignElgamalHash(priv, signer.Hash(rawTx))
	if err != nil {
		t.Fatalf("SignElgamalHash: %v", err)
	}
...
func TestUNOTxPoolExecutionRejectParityUnshieldZeroAmount(t *testing.T) {
	t.Parallel()

	pool, _ := setupTxPool()
	defer pool.Stop()

	tx, from, pub := testUNOElgamalTx(t, 0, 1_200_000, testUNOUnshieldWireZeroAmount(t))
	testSetElgamalSigner(pool, from, pub)
	testSetBalanceExact(pool, from, new(big.Int).Set(tx.Cost()))
...
func TestTransactionNegativeValue(t *testing.T) {
	...
	signer := types.LatestSignerForChainID(params.TestChainConfig.ChainID)
	tx, _ := signTestSignerTx(signer, key, 0, common.Address{}, big.NewInt(-1), 100, big.NewInt(1), nil)
	from, _ := deriveSender(tx)
	testAddBalance(pool, from, big.NewInt(1))
...
func TestTransactionDoubleNonce(t *testing.T) {
	...
	signer := types.LatestSignerForChainID(params.TestChainConfig.ChainID)
	tx1, _ := signTestSignerTx(signer, key, 0, common.Address{}, big.NewInt(100), 100000, big.NewInt(1), nil)
	tx2, _ := signTestSignerTx(signer, key, 0, common.Address{}, big.NewInt(100), 1000000, big.NewInt(2), nil)
	tx3, _ := signTestSignerTx(signer, key, 0, common.Address{}, big.NewInt(100), 1000000, big.NewInt(1), nil)
