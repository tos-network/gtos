...
// https://github.com/tos-network/gtos/pull/15549.
...
func (test *snapshotTest) checkEqual(state, checkstate *StateDB) error {
	for _, addr := range test.addrs {
		...
		checkeq("GetByteCode", state.GetByteCode(addr), checkstate.GetByteCode(addr))
		checkeq("GetByteCodeHash", state.GetByteCodeHash(addr), checkstate.GetByteCodeHash(addr))
		checkeq("GetCodeSize", state.GetCodeSize(addr), checkstate.GetCodeSize(addr))
		checkeq("GetByteCodeSize", state.GetByteCodeSize(addr), checkstate.GetByteCodeSize(addr))
...
// See https://github.com/tos-network/gtos/pull/15225#issuecomment-380191512
...
//
// See https://github.com/tos-network/gtos/issues/20106.
func TestCopyCommitCopy(t *testing.T) {
	...
	state.SetByteCode(addr, []byte("hello byte")) // Change an external metadata
	...
	if bytecode := state.GetByteCode(addr); !bytes.Equal(bytecode, []byte("hello byte")) {
		t.Fatalf("initial code mismatch: have %x, want %x", bytecode, []byte("hello byte"))
	}
	...
	if bytecode := copyOne.GetByteCode(addr); !bytes.Equal(bytecode, []byte("hello byte")) {
		t.Fatalf("first copy pre-commit code mismatch: have %x, want %x", bytecode, []byte("hello byte"))
	}
	...
	if bytecode := copyOne.GetByteCode(addr); !bytes.Equal(bytecode, []byte("hello byte")) {
		t.Fatalf("first copy post-commit code mismatch: have %x, want %x", bytecode, []byte("hello byte"))
	}
	if val := copyOne.GetState(addr, skey); val != sval {
	...
	if bytecode := copyTwo.GetByteCode(addr); !bytes.Equal(bytecode, []byte("hello byte")) {
		t.Fatalf("second copy code mismatch: have %x, want %x", bytecode, []byte("hello byte"))
	}
...
func TestCopyCopyCommitCopy(t *testing.T) {
	...
	if bytecode := state.GetByteCode(addr); !bytes.Equal(bytecode, []byte("hello byte")) {
		t.Fatalf("initial bytecode mismatch: have %x, want %x", bytecode, []byte("hello byte"))
	}
	if val := state.GetState(addr, skey); val != sval {
		t.Fatalf("initial non-committed storage slot mismatch: have %x, want %x", val, sval)
	}
	...
	if bytecode := copyOne.GetByteCode(addr); !bytes.Equal(bytecode, []byte("hello byte")) {
		t.Fatalf("first copy bytecode mismatch: have %x, want %x", bytecode, []byte("hello byte"))
	}
	if val := copyOne.GetState(addr, skey); val != sval {
	...
	if bytecode := copyTwo.GetByteCode(addr); !bytes.Equal(bytecode, []byte("hello byte")) {
		t.Fatalf("second copy pre-commit bytecode mismatch: have %x, want %x", bytecode, []byte("hello byte"))
	}
	if val := copyTwo.GetState(addr, skey); val != sval {
	...
	if bytecode := copyTwo.GetByteCode(addr); !bytes.Equal(bytecode, []byte("hello byte")) {
		t.Fatalf("second copy post-commit bytecode mismatch: have %x, want %x", bytecode, []byte("hello byte"))
	}
	if val := copyTwo.GetState(addr, skey); val != sval {
	...
	if bytecode := copyThree.GetByteCode(addr); !bytes.Equal(bytecode, []byte("hello byte")) {
		t.Fatalf("third copy bytecode mismatch: have %x, want %x", bytecode, []byte("hello byte"))
	}
...
func TestMissingTrieNodes(t *testing.T) {
	...
	addr := toAddr([]byte("so"))
	{
		...
		state.SetByteCode(addr, []byte{2, 3, 4})
		a2 := toAddr([]byte("another"))
		state.SetBalance(a2, big.NewInt(100))
		state.SetCode(a2, []byte{1, 2, 4})
		state.SetByteCode(a2, []byte{4, 5, 6})
		root, _ = state.Commit(false)
		t.Logf("root: %x", root)
		// force-flush
