...
func CheckDanglingStorage(chaindb tosdb.KeyValueStore) error {
...
func checkDanglingDiskStorage(chaindb tosdb.KeyValueStore) error {
	var (
		lastReport = time.Now()
		start      = time.Now()
		lastKey    []byte
...
// snapshot difflayers.
func checkDanglingMemStorage(db tosdb.KeyValueStore) error {
	start := time.Now()
...
func CheckJournalAccount(db tosdb.KeyValueStore, hash common.Hash) error {
