...
func NewPruner(db tosdb.Database, datadir, trieCachePath string, bloomSize uint64) (*Pruner, error) {
...
func prune(snaptree *snapshot.Tree, root common.Hash, maindb tosdb.Database, stateBloom *stateBloom, bloomPath string, middleStateRoots map[common.Hash]struct{}, start time.Time) error {
...
// in the disk.
func RecoverPruning(datadir string, db tosdb.Database, trieCachePath string) error {
...
func extractGenesis(db tosdb.Database, stateBloom *stateBloom) error {
...
pruning. Remember don't start the GTOS without deleting the clean trie cache
otherwise the entire database may be damaged!

Check the command description "gtos snapshot prune-state --help" for more details.
`
