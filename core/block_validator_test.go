...
func TestHeaderVerification(t *testing.T) {
	...
	var (
		...
		blocks, _ = GenerateChain(params.TestChainConfig, genesis, tosash.NewFaker(), testdb, 8, nil)
	)
	...
	chain, _ := NewBlockChain(testdb, nil, params.TestChainConfig, tosash.NewFaker(), nil, nil)
	defer chain.Stop()
	...
	for i := 0; i < len(blocks); i++ {
		for j, valid := range []bool{true, false} {
			...
			if valid {
				...
				engine := tosash.NewFakeFailer(headers[i].Number.Uint64())
				_, results = engine.VerifyHeaders(chain, []*types.Header{headers[i]}, []bool{true})
			}
...
func testHeaderVerificationForMerging(t *testing.T, isClique bool) {
	...
	// Run the header checker for blocks one-by-one, checking for both valid and invalid nonces
	chain, _ := NewBlockChain(testdb, nil, chainConfig, runEngine, nil, nil)
	defer chain.Stop()
...
func testHeaderConcurrentVerification(t *testing.T, threads int) {
	...
	for i, valid := range []bool{true, false} {
		...
		if valid {
			...
			chain, _ := NewBlockChain(testdb, nil, params.TestChainConfig, tosash.NewFakeFailer(uint64(len(headers)-1)), nil, nil)
			_, results = chain.engine.VerifyHeaders(chain, headers, seals)
			chain.Stop()
		}
...
func testHeaderConcurrentAbortion(t *testing.T, threads int) {
	...
	var (
		...
		gspec     = &Genesis{Config: params.TestChainConfig}
		genesis   = gspec.MustCommit(testdb)
		blocks, _ = GenerateChain(params.TestChainConfig, genesis, tosash.NewFaker(), testdb, 1024, nil)
	)
	headers := make([]*types.Header, len(blocks))
	seals := make([]bool, len(blocks))
	...
	chain, _ := NewBlockChain(testdb, nil, params.TestChainConfig, tosash.NewFakeDelayer(time.Millisecond), nil, nil)
	defer chain.Stop()
