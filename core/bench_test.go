...
// genTxRing returns a block generator that sends tos in a ring
...
func benchInsertChain(b *testing.B, disk bool, gen func(int, *BlockGen)) {
	...
	chainman, _ := NewBlockChain(db, nil, gspec.Config, dpos.NewFaker(), nil, nil)
	defer chainman.Stop()
	b.ReportAllocs()
	b.ResetTimer()
...
// into a database.
func makeChainForBench(db tosdb.Database, full bool, count uint64) {
...
func benchReadChain(b *testing.B, full bool, count uint64) {
	...
	for i := 0; i < b.N; i++ {
		...
		chain, err := NewBlockChain(db, &cacheConfig, params.TestChainConfig, dpos.NewFaker(), nil, nil)
		if err != nil {
