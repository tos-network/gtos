...
func mustDecodeLegacyTransaction(t *testing.T, encoded string) *Transaction {
	t.Helper()
	var tx Transaction
	if err := tx.UnmarshalBinary(common.FromHex(encoded)); err != nil {
...
func TestBlockEncoding(t *testing.T) {
	t.Skip("legacy/accesslist block fixtures are removed in signer-tx-only mode")
...
func TestDynamicFeeBlockEncoding(t *testing.T) {
	t.Skip("legacy/dynamic-fee block fixtures are removed in signer-tx-only mode")
	...
	txdata := &SignerTx{
		...
		To:         &to,
		Gas:        123457,
		GasPrice:   new(big.Int).Set(block.BaseFee()),
		Value:      big.NewInt(0),
		AccessList: accesses,
		Data:       []byte{},
		From:       common.Address{},
...
func TestTypedEnvelopeBlockEncoding(t *testing.T) {
	t.Skip("legacy/accesslist block fixtures are removed in signer-tx-only mode")
	...
	check("Transactions[0].Hash", block.Transactions()[0].Hash(), tx1.Hash())
	check("Transactions[1].Hash", block.Transactions()[1].Hash(), tx2.Hash())
	check("Transactions[1].Type()", block.Transactions()[1].Type(), uint8(SignerTxType))
