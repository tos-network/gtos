...
func TestStoredReceiptDecoding(t *testing.T) {
	tests := []struct {
		name   string
...
func TestDeriveFields(t *testing.T) {
	...
	txs := Transactions{
		...
		NewTransaction(2, to2, big.NewInt(2), 2, big.NewInt(2), nil),
		NewTransaction(3, to3, big.NewInt(3), 3, big.NewInt(3), nil),
	}
...
func TestTypedReceiptEncodingDecoding(t *testing.T) {
	...
	for _, r := range bundleWant {
		r.Bloom = CreateBloom(Receipts{r})
	}
	payload, err := rlp.EncodeToBytes(bundleWant)
	if err != nil {
		t.Fatal(err)
	}
	...
	check := func(bundle []*Receipt) {
		...
		for i, receipt := range bundle {
			if got, want := receipt.Type, uint8(SignerTxType); got != want {
...
func TestReceiptMarshalBinary(t *testing.T) {
	...
	have, err := receipt.MarshalBinary()
	if err != nil {
		t.Fatalf("marshal binary error: %v", err)
	}
	buf := new(bytes.Buffer)
	Receipts{receipt}.EncodeIndex(0, buf)
	...
	if err := rlp.DecodeBytes(buf.Bytes(), &encodedPayload); err != nil {
		t.Fatalf("decode rlp error: %v", err)
	}
	if !bytes.Equal(have, encodedPayload) {
		t.Errorf("BinaryMarshal and EncodeRLP payload mismatch, got %x want %x", have, encodedPayload)
	}
}
...
func TestReceiptUnmarshalBinary(t *testing.T) {
	...
	if !reflect.DeepEqual(decoded, receipt) {
		t.Errorf("receipt unmarshalled from binary mismatch, got %v want %v", decoded, receipt)
	}

	untypedReceiptBinary := common.FromHex("c0")
	if err := new(Receipt).UnmarshalBinary(untypedReceiptBinary); err != ErrTxTypeNotSupported {
		t.Fatalf("untyped receipt should be rejected, got %v", err)
	}
}
