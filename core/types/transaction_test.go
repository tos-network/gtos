...
func mustDecodeTxHex(input string) *Transaction {
	var tx Transaction
	if err := tx.UnmarshalBinary(common.FromHex(input)); err != nil {
		panic(err)
	}
	return &tx
}
...
func TestTransactionSigHash(t *testing.T) {
	...
	if signer.Hash(emptyTx) == (common.Hash{}) {
		t.Fatal("empty transaction signing hash should not be empty")
	}
	if signer.Hash(rightvrsTx) == (common.Hash{}) {
		t.Fatal("legacy transaction signing hash should not be empty")
	}
	if signer.Hash(emptyTx) == signer.Hash(rightvrsTx) {
		t.Fatal("different transactions must have different signing hashes")
	}
}
