...
func TestRemoteNotify(t *testing.T) {
	...
	tosash := NewTester([]string{server.URL}, false)
	defer tosash.Close()
	...
	tosash.Seal(nil, block, nil, nil)
	select {
	case work := <-sink:
		if want := tosash.SealHash(header).Hex(); work[0] != want {
...
func TestRemoteNotifyFull(t *testing.T) {
	...
	tosash := New(config, []string{server.URL}, false)
	defer tosash.Close()
...
func TestRemoteMultiNotify(t *testing.T) {
	...
	// Create the custom ethash engine.
	tosash := NewTester([]string{server.URL}, false)
	tosash.config.Log = testlog.Logger(t, log.LvlWarn)
	defer tosash.Close()
...
func TestStaleSubmission(t *testing.T) {
	...
	for id, c := range testcases {
		for _, h := range c.headers {
			tosash.Seal(nil, types.NewBlockWithHeader(h), results, nil)
		}
		if res := api.SubmitWork(fakeNonce, tosash.SealHash(c.headers[c.submitIndex]), fakeDigest); res != c.submitRes {
