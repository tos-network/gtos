...
MAX_VERIFY_NS="${PRIV_MAX_VERIFY_NS:-1500000}"
MAX_VERIFY_BOP="${PRIV_MAX_VERIFY_BOP:-65536}"
MAX_VERIFY_RATIO="${PRIV_MAX_VERIFY_RATIO:-64}"

tmp_out="$(mktemp)"
trap 'rm -f "${tmp_out}"' EXIT

echo "==> priv gas-griefing audit benchmarks (cgo+ed25519c)"
CGO_ENABLED=1 go test -tags ed25519c ./core -run '^$' \
  -bench 'BenchmarkPrivShieldInvalidProof(Shape|VerifyPath)$' \
  -benchtime=300x -count=1 | tee "${tmp_out}"

shape_ns="$(awk '/BenchmarkPrivShieldInvalidProofShape/ {print $3}' "${tmp_out}" | tail -n1)"
shape_bop="$(awk '/BenchmarkPrivShieldInvalidProofShape/ {print $5}' "${tmp_out}" | tail -n1)"
verify_ns="$(awk '/BenchmarkPrivShieldInvalidProofVerifyPath/ {print $3}' "${tmp_out}" | tail -n1)"
verify_bop="$(awk '/BenchmarkPrivShieldInvalidProofVerifyPath/ {print $5}' "${tmp_out}" | tail -n1)"
